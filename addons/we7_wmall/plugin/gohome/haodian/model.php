...
{
	$data = array(
		1 => array('text' => '入驻中', 'css' => 'label label-success'),
		2 => array('text' => '暂停中', 'css' => 'label label-warning'),
		3 => array('text' => '已到期', 'css' => 'label label-warning'),
		4 => array('text' => '未通过', 'css' => 'label label-default'),
		5 => array('text' => '待审核', 'css' => 'label label-danger'),
		6 => array('text' => '待入驻', 'css' => 'label label-info')
		);
...
function haodian_get_activity($sid)
{
	global $_W;
	$table = array(
		'kanjia'  => array('name' => 'tiny_wmall_kanjia', 'field' => 'name, price, oldprice '),
		'pintuan' => array('name' => 'tiny_wmall_pintuan_goods', 'field' => 'name, price, oldprice, peoplenum '),
		'seckill' => array('name' => 'tiny_wmall_seckill_goods', 'field' => 'name, price, oldprice ')
		);
	...
	foreach ($table as $key => $value) {
		...
		if (!empty($temp)) {
			if ($key == 'kanjia') {
				...
				$data[$key]['text'] = $temp['name'] . ' 原价¥' . $temp['oldprice'] . ' 邀请好友帮忙砍价最低¥' . $temp['price'] . '即可购买';
			}
			else if ($key == 'pintuan') {
				...
				if ($key == 'seckill') {
					...
					$data[$key]['text'] = $temp['name'] . ' 原价¥' . $temp['oldprice'] . ' 限时抢购价¥' . $temp['price'] . '即可购买';
				}
			}
		}
	}
...
function haodian_settle_order_bill($order)
{
	...
	if (0 < $order['agentid']) {
		...
		$agent_serve['final'] = '(代理商抽取佣金 ￥' . $order['final_fee'] . ' - 平台服务佣金 ￥' . $agent_serve_fee . ')';
