...
{
    global $_W;
    $categorys = pdo_fetchall("select * from " . tablename("tiny_wmall_seckill_goods_category") . " where uniacid = :uniacid order by displayorder desc", array(":uniacid" => $_W["uniacid"]), "id");
    if (!empty($categorys)) {
    ...
    if ($type == "status") {
        ...
        if ($type == "confirm") {
            ...
            if (!empty($credit1_config) && $credit1_config["status"] == 1 && 0 < $credit1_config["grant_num"] && 0 < $order["uid"]) {
                ...
                if (0 < $credit1) {
                    mload()->model("member");
                    $result = member_credit_update($order["uid"], "credit1", $credit1, array(0, "生活圈抢购订单完成, 赠送" . $credit1 . "积分"));
                    if (is_error($result)) {
