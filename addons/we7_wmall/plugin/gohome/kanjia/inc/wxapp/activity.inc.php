...
if ($op == 'detail') {
	...
	if (!empty($member_takeinfo)) {
		$take_status = 1;
		$_W['_share'] = array('title' => '砍价进行时', 'desc' => '我正在参加最低' . $activity['price'] . '元购买' . $activity['name'] . '的活动，快来帮我砍价吧', 'imgUrl' => $activity['thumb'], 'link' => ivurl('/gohome/pages/kanjia/share', array('activityid' => $activity['id'], 'uid' => $member_takeinfo['uid']), true));
		$sharedata = array('title' => '砍价进行时', 'desc' => '我正在参加最低' . $activity['price'] . '元购买' . $activity['name'] . '的活动，快来帮我砍价吧', 'imageUrl' => $activity['thumb'], 'path' => '/gohome/pages/kanjia/share?activityid=' . $activity['id'] . '&uid=' . $member_takeinfo['uid']);
	}
	else {
		$_W['_share'] = array('title' => $activity['share']['share_title'], 'desc' => $activity['share']['share_detail'], 'imgUrl' => tomedia($activity['share']['share_thumb']), 'link' => ivurl('/gohome/pages/kanjia/detail', array('id' => $activity['id']), true));
		$sharedata = array('title' => $activity['share']['share_title'], 'desc' => $activity['share']['share_detail'], 'imageUrl' => tomedia($activity['share']['share_thumb']), 'path' => '/gohome/pages/kanjia/detail?id=' . $activity['id']);
	}
...
if ($op == 'share') {
	...
	$_W['_share'] = array('title' => '砍价进行时', 'desc' => '我正在参加最低' . $activity['price'] . '元购买' . $activity['name'] . '的活动，快来帮我砍价吧', 'imgUrl' => $activity['thumb'], 'link' => ivurl('/gohome/pages/kanjia/share', array('activityid' => $activity['id'], 'uid' => $takeinfo['uid']), true));
	$sharedata = array('title' => '砍价进行时', 'desc' => '我正在参加最低' . $activity['price'] . '元购买' . $activity['name'] . '的活动，快来帮我砍价吧', 'imageUrl' => $activity['thumb'], 'path' => '/gohome/pages/kanjia/share?activityid=' . $activity['id'] . '&uid=' . $takeinfo['uid']);
