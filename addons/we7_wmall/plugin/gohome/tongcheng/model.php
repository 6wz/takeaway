...
function tongcheng_tiezi_notice($tieziOrId, $type, $extra = array(  ))
{
    ...
    if( $channel == "wap" ) 
    {
        $first = array( "like" => "您发表的帖子有新的点赞", "comment" => "您发表的帖子有新的评论", "reply" => "您发表的帖子有新的评论回复" );
    ...
    if( is_error($status) ) 
    {
        slog("wxtplNotice", "同城帖子点赞、评论、回复通知发帖人-帖子id:" . $tiezi["id"], $send, $status["message"]);
    }
