...
function creditshop_order_notice($orderOrId, $type, $extra = array(  ))
{
    ...
    if( $channel == "wap" ) 
    {
        if( $type == "pay" ) 
        {
            $params = array( "first" => "您在积分商城兑换了" . $goods_types[$order["goods_type"]], "keyword1" => (string) $goods_types[$order["goods_type"]] . ":" . $order["goods_info"]["title"], "keyword2" => date("Y-m-d H:i", $order["paytime"]), "keyword3" => "积分商城积分兑换" );
        }
        else
        {
            if( $type == "handle" ) 
    ...
    else
    {
        if( $channel == "wxapp" ) 
        {
            ...
            if( $type == "pay" ) 
            {
                $params = array( "keyword1" => "积分商城下单", "keyword2" => (string) $goods_types[$order["goods_type"]] . ":" . $order["goods_info"]["title"], "keyword3" => "订单" . $status_cn[$order["status"]], "keyword4" => date("Y-m-d H:i", $order["paytime"]), "keyword5" => "您在积分商城兑换了" . $order["goods_info"]["title"] );
            }
            else
            {
                if( $type == "handle" ) 
                {
                    $params = array( "keyword1" => "积分商城订单核销", "keyword2" => "商品:" . $order["goods_info"]["title"], "keyword3" => "订单已核销", "keyword4" => date("Y-m-d H:i", TIMESTAMP), "keyword5" => "您在积分商城兑换的" . $order["goods_info"]["title"] . "已核销" );
                }
...
function creditshop_order_manager_notice($orderOrId, $type, $extra = array(  ))
{
    ...
    if( is_error($status) ) 
    {
        slog("wxtplNotice", "积分商城订单通知平台管理员", $send, $status["message"]);
    }
