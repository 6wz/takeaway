...
function ordergrant_grant($id)
{
	...
	$is_exist = pdo_fetch('select id from ' . tablename('tiny_wmall_order_grant_record') . ' where uniacid = :uniacid and uid = :uid and oid = :oid and addtime > :time1 and addtime < :time2 and type <= 1', array(':uniacid' => $_W['uniacid'], ':uid' => $order['uid'], ':oid' => $order['id'], ':time1' => $time1, ':time2' => $time2));
	...
	if (empty($is_exist)) {
		...
		$result = member_credit_update($order['uid'], $config_ordergrant['grantType'], $insert['grant'], $log);

		if (is_error($result)) {
			slog('ordergrant', '下单有礼', array('order_id' => $order['id'], 'uid' => $order['uid'], 'credit_type' => $config_ordergrant['grantType'], 'reason_from' => 'ordergrant'), $result['message']);
		}

		pdo_insert('tiny_wmall_order_grant_record', $insert);
	}

	$is_exist = pdo_fetch('select id from ' . tablename('tiny_wmall_order_grant_record') . ' where uniacid = :uniacid and uid = :uid and oid = :oid and addtime > :time1 and addtime < :time2 and type = 4', array(':uniacid' => $_W['uniacid'], ':uid' => $order['uid'], ':oid' => $order['id'], ':time1' => $time1, ':time2' => $time2));
	if (!empty($config_ordergrant['special']) && empty($is_exist)) {
		foreach ($config_ordergrant['special'] as $special) {
			if ($special['date'] == date('Y-m-d')) {
				...
				$log = array($order['uid'], $insert['mark']);
				$result = member_credit_update($order['uid'], $config_ordergrant['grantType'], $special['grant'], $log);
