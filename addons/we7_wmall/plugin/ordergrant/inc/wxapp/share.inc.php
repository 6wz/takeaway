...
if ($op == 'list') {
	...
	if (!empty($comments)) {
		foreach ($comments as &$row) {
			...
			if (0 < $row['activity']['num']) {
				$num = 0;

				foreach ($row['activity']['items'] as &$item) {
					++$num;
					$item['int_num'] = $num;
				}
			}
...
if ($op == 'detail') {
	...
	$activity = array_values($activity['items']);
	...
	$comment['score'] = round(($comment['delivery_service'] + $comment['goods_quality']) / 2, 2);
	...
	$result = array('order' => $order, 'store' => $store, 'activity' => $activity, 'goods' => $goods, 'discounts' => $discounts, 'comment' => $comment, 'hot_goods' => $hot_goods);
...
if ($op == 'grant') {
	...
	if (TIMESTAMP - $order['endtime'] < $config_share['share_grant_days_limit'] * 86400) {
		...
		if (0 < $grant) {
			$insert = array('uniacid' => $_W['uniacid'], 'uid' => $order['uid'], 'oid' => $id, 'grant' => $grant, 'credittype' => $config_share['grantType'], 'type' => 5, 'stat_month' => date('Ym'), 'addtime' => TIMESTAMP, 'mark' => '分享订单奖励' . $grant . $config_share['grantType_cn']);
			pdo_insert('tiny_wmall_order_grant_record', $insert);
			$log = array($order['uid'], $insert['mark']);
			mload()->model('member');
			member_credit_update($order['uid'], $config_share['grantType'], $grant, $log);
		}
	}
