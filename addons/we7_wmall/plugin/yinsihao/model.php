...
function yinsihao_bind($orderOrId, $type, $ordersn)
{
    global $_W;
    global $_GPC;
    $order = $orderOrId;
    ...
    $status = yinsihao_order_check($order);
    if (empty($status)) {
        return error(-2, "订单已无法使用隐私号联系");
    }
    $types = array("store", "deliveryer", "member");
...
function yinsihao_order_check($orderOrId)
{
    global $_W;
    $order = $orderOrId;
    if (!is_array($order)) {
