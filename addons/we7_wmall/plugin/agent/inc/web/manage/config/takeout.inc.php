...
if ($op == 'range') {
	...
	if ($_W['ispost']) {
		$range = array(
			'map'          => array('location_x' => trim($_GPC['map']['lat']), 'location_y' => trim($_GPC['map']['lng'])),
			'city'         => trim($_GPC['city']),
...
if ($op == 'order') {
	...
	if ($_W['ispost']) {
		$order = array('show_no_pay' => intval($_GPC['show_no_pay']), 'auto_refresh' => intval($_GPC['auto_refresh']), 'deliveryer_collect_notify_clerk' => intval($_GPC['deliveryer_collect_notify_clerk']), 'dispatch_mode' => intval($_GPC['dispatch_mode']), 'can_collect_order' => intval($_GPC['can_collect_order']), 'deliveryer_transfer_reason' => explode('
