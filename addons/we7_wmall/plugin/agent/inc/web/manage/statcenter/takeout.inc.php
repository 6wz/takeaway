...
if ($op == 'index') {
	...
	if ($_W['isajax']) {
		...
		$stat['total_agent_discount_fee'] = floatval(pdo_fetchcolumn('select round(sum(agent_discount_fee), 2) from ' . tablename('tiny_wmall_order') . $condition . ' and status = 5 and is_pay = 1', $params));
		...
		$chart = array(
			'stat'   => $stat,
			'fields' => array('total_success_order', 'total_fee', 'final_fee', 'plateform_delivery_fee', 'plateform_serve_fee', 'plateform_deliveryer_fee', 'agent_discount_fee', 'plateform_discount_fee', 'refund_fee', 'extra_fee'),
			'titles' => array('有效订单量', '营业总额', '总入账', '平台配送费收入', '佣金收入', '配送员配送费支出', '代理补贴', '平台补贴', '总退款', '附加费')
			);
		...
		$records = pdo_fetchall('SELECT stat_day, count(*) as total_success_order,round(sum(total_fee), 2) as total_fee, round(sum(final_fee), 2) as final_fee, round(sum(plateform_delivery_fee), 2) as plateform_delivery_fee, round(sum(plateform_deliveryer_fee), 2) as plateform_deliveryer_fee, round(sum(plateform_serve_fee), 2) as plateform_serve_fee, round(sum(agent_discount_fee), 2) as agent_discount_fee, round(sum(plateform_discount_fee), 2) as plateform_discount_fee, round(sum(refund_fee), 2) as refund_fee, round(sum(extra_fee), 2) as extra_fee
	...
	$records_temp = pdo_fetchall('SELECT stat_day,
		...
		round(sum(agent_final_fee), 2) as agent_final_fee,
	...
	while ($starttime <= $i) {
		if (empty($records_temp[$i])) {
			$records[] = array('stat_day' => $i, 'total_success_order' => 0, 'total_fee' => 0, 'final_fee' => 0, 'plateform_serve_fee' => 0, 'plateform_deliveryer_fee' => 0, 'plateform_delivery_fee' => 0, 'plateform_discount_fee' => 0, 'agent_discount_fee' => 0, 'agent_serve_fee' => 0, 'agent_final_fee' => 0, 'refund_fee' => 0, 'extra_fee' => 0);
		}
		else {
