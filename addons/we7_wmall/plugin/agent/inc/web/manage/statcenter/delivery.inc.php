...
if ($op == 'index') {
	$_W['page']['title'] = '配送员统计';
	$condition = ' as a left join ' . tablename('tiny_wmall_order_comment') . ' as b on a.id = b.oid where a.uniacid = :uniacid and a.agentid = :agentid and a.status = 5 and a.delivery_type = 2 and a.deliveryer_id != 0 and a.order_type <= 2';
	...
	$condition_takegoods_timeout = $condition . ' and (a.delivery_takegoods_time - a.delivery_assign_time) > ' . $config_takegoods_timeout;
	$condition_service_timeout = $condition . ' and (a.delivery_success_time - a.delivery_takegoods_time) > ' . $config_service_timeout;
	...
	if ($_W['isajax']) {
		...
		$chart = array(
			'stat'   => $stat,
			'fields' => array('total_success_order', 'total_normal_order', 'total_timeout_order', 'total_before_order', 'avg_normal_delivery_time', 'total_takegoods_timeout_order', 'total_service_timeout_order'),
	...
	$deliveryers = deliveryer_fetchall(0, array('work_status' => -1));
