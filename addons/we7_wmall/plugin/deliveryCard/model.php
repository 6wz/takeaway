...
', $remark));
	...
	if (!empty($order['openid'])) {
		...
		$params = array('first' => $tips, 'OrderSn' => $order['ordersn'], 'OrderStatus' => '已生效', 'remark' => implode('
', $remark));
		$url = ivurl('pages/deliveryCard/index', array(), true);
		$send = sys_wechat_tpl_format($params);
...
function card_setmeal_buy($order_id, $extra = array())
{
	global $_W;
	...
	if (!$order['is_pay']) {
		...
		if (!empty($card)) {
			...
			if (!empty($member)) {
				...
				$orgin_starttime = strtotime(date('Y-m-d'));

				if ($starttime < $member['setmeal_endtime']) {
					$starttime = $member['setmeal_endtime'];
					$orgin_starttime = $member['setmeal_starttime'];
				}

				$endtime = strtotime($card['days'] . 'days', $starttime);
				pdo_update('tiny_wmall_delivery_cards_order', array('pay_type' => $extra['type'], 'is_pay' => 1, 'paytime' => TIMESTAMP, 'starttime' => $starttime, 'endtime' => $endtime), array('id' => $order['id'], 'uniacid' => $_W['uniacid']));
				pdo_update('tiny_wmall_members', array('setmeal_id' => $card['id'], 'setmeal_day_free_limit' => $card['day_free_limit'], 'setmeal_deliveryfee_free_limit' => $card['delivery_fee_free_limit'], 'setmeal_starttime' => $orgin_starttime, 'setmeal_endtime' => $endtime), array('uid' => $member['uid'], 'uniacid' => $_W['uniacid']));
