		...
		if (!empty($rule)) {
			...
			if ($store['data']['zhunshibao']['fee_type'] == 1) {
				...
				if ($store['data']['zhunshibao']['fee_type'] == 2) {
					...
					$log['compensate_fee'] = '￥' . $update['zhunshibao_compensate'];
					$log['compensate_fee_cn'] = '（订单支付 ￥' . $order_final_fee . ' * ' . $rule['fee'] . '%）';
...
function zhunshibao_update_status($orderOrId, $status)
{
	...
	if ($status == 3) {
		$log = array($order['uid'], '准时宝赔偿, 订单号:' . $order['ordersn'] . ', 赔付金额:' . $order['zhunshibao_compensate'] . '元', 'we7_wmall');
		mload()->model('member');
		member_credit_update($order['uid'], 'credit2', $order['zhunshibao_compensate'], $log);

		if (0 < $order['agentid']) {
			mload()->model('agent');
			$remark = '准时宝赔付' . $order['zhunshibao_compensate'] . '元,订单id：' . $order['id'];
