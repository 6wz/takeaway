...
{
	$rank = array(
		'one'   => array('css' => 'label label-success', 'text' => '一等奖', 'color' => ''),
		'two'   => array('css' => 'label label-primary', 'text' => '二等奖', 'color' => ''),
		'three' => array('css' => 'label label-warning', 'text' => '三等奖', 'color' => '')
		);
...
{
	$type = array(
		'credit1'   => array('css' => 'label label-danger', 'text' => '积分', 'name' => 'credit1'),
		'credit2'   => array('css' => 'label label-primary', 'text' => '余额', 'name' => 'credit2'),
		'redpacket' => array('css' => 'label label-success', 'text' => '红包', 'name' => 'redpacket'),
		'gift'      => array('css' => 'label label-success', 'text' => '赠品', 'name' => 'gift')
		);
	return $type[$key];
}
...
function get_wheel_url($extra = array(), $type = 'pay')
{
	...
	if (!empty($_wheel_scene[$type]) && !empty($_wheel_scene[$type]['wheel_id'])) {
		...
		if ($wheel['starttime'] <= TIMESTAMP && TIMESTAMP < $wheel['endtime']) {
			...
			else if ($extra['ochannel'] == 'wap') {
				$url = ivurl('/pages/wheel/index', array('id' => $_wheel_scene[$type]['wheel_id'], 'order_id' => $order_id));
			}
			else if ($extra['ochannel'] == 'h5app') {
				$url = ivurl('/pages/wheel/index', array('id' => $_wheel_scene[$type]['wheel_id'], 'order_id' => $order_id), true);
			}
			else {
