	...
	if ($type == 'pay') {
		...
		if (!empty($meal_redpackets_data)) {
			...
			if (!empty($openid)) {
				...
				$params = array('first' => '您在' . $config['mall']['title'] . '的账户有' . $num . '个代金券到账', 'keyword1' => '账户代金券', 'keyword2' => $discount . '元', 'keyword3' => date('Y-m-d H:i', TIMESTAMP), 'keyword4' => '账户有新的代金券到账', 'remark' => implode('
