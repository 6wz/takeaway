...
{
	$data = array(
		1 => array('css' => 'label label-success', 'text' => '正常'),
		2 => array('css' => 'label label-default', 'text' => '黑名单'),
		0 => array('css' => 'label label-warning', 'text' => '待审核')
		);
...
{
	$data = array(
		'wechat' => array('css' => 'label label-success', 'text' => '微信'),
		'credit' => array('css' => 'label label-warning', 'text' => '账户余额')
		);
...
function spread_group_update($spread_id = 0, $wx_tpl = false)
{
	...
	else if ($relate['group_update_mode'] == 'order_money_1') {
	...
	else if ($relate['group_update_mode'] == 'order_count') {
		$condition .= ' and is_pay = 1 and status = 5 and (spread1 = :spread or spread2 = :spread)';
		$params[':spread'] = $spread_id;
	...
	else if ($relate['group_update_mode'] == 'order_count_1') {
		$condition .= ' and is_pay = 1 and status = 5 and spread1 = :spread';
		$params[':spread'] = $spread_id;
		$result = pdo_fetchcolumn('select count(*) from' . tablename('tiny_wmall_order') . $condition, $params);
	...
	else if ($relate['group_update_mode'] == 'self_order_money') {
	...
	else if ($relate['group_update_mode'] == 'self_order_count') {
		$condition .= ' and is_pay = 1 and status =5 and uid = :uid';
		$params[':uid'] = $spread_id;
		$result = pdo_fetchcolumn('select count(*) from' . tablename('tiny_wmall_order') . $condition, $params);
		$spread_paotui = $spread_gohome = 0;
	...
	else if ($relate['group_update_mode'] == 'down_count') {
	...
	if ($spread_info['admin_update_rules'] == 'only_up') {
		if ($result < $group_condition_now) {
			$group_id = $spread_info['spread_groupid'];
		}
	}
...
function sys_notice_spread_down($uid, $type, $extra = array())
{
	...
	if ($type == 'pseudo_down') {
		...
		if ($type == 'new_down') {
			...
			else if ($extra['channel'] == 'order_end') {
			...
			$remark = array('顾客昵称: ' . $member['nickname'], $member['nickname'] . '今后在' . $config_mall['title'] . '下单,您将会获得平台的返佣');
		}
	}
...
', $remark);
	$send = array(
		...
		'keyword1' => array('value' => $member['uid'], 'color' => '#ff510'),
		'keyword2' => array('value' => date('Y-m-d H:i', $member['addtime']), 'color' => '#ff510'),
...
', $remark);
			$send = array(
				'first'    => array('value' => $tips, 'color' => '#ff510'),
				'keyword1' => array('value' => $member['realname'], 'color' => '#ff510'),
				'keyword2' => array('value' => $member['realname'], 'color' => '#ff510'),
		...
		if (!empty($commission)) {
			if ($config_spread['basic']['commission_from'] == 1) {
				if (0 < $order['plateform_serve_fee']) {
					...
					$commission['spread1'] = $commission['spread2'] = 0;
				}
			...
			if (0 <= $fee_spread1) {
				...
				if ($commission['commission1_type'] == 'ratio') {
					$extra['remark'] = $router['order_type_cn'] . ' 订单号:' . $order['id'] . ', 一级下线佣金费率:' . $commission['spread1_rate'] . ', 佣金:' . $fee_spread1 . '元';
				}
				else {
					if ($commission['commission1_type'] == 'fixed') {
						$extra['remark'] = $router['order_type_cn'] . ' 订单号:' . $order['id'] . ', 一级下线佣金:' . $fee_spread1 . '元';
					}
				}
			...
			if (0 <= $fee_spread2) {
				...
				if ($commission['commission2_type'] == 'ratio') {
					$extra['remark'] = $router['order_type_cn'] . ' 订单号:' . $order['id'] . ', 二级下线佣金费率:' . $commission['spread2_rate'] . ', 佣金:' . $fee_spread2 . '元';
				}
				else {
					if ($commission['commission2_type'] == 'fixed') {
						$extra['remark'] = $router['order_type_cn'] . ' 订单号:' . $order['id'] . ', 二级下线佣金:' . $fee_spread2 . '元';
					}
				}
...
{
	$data = array(
		1 => array('css' => 'label label-success', 'text' => '推广佣金入账'),
		2 => array('css' => 'label label-danger', 'text' => '申请提现'),
		3 => array('css' => 'label label-default', 'text' => '其他变动')
		);
...
function member_spread_confirm($order_id)
{
	...
	if ($config_spread_relate['become_child'] == 1) {
		...
		if ($config_spread_relate['become_child'] == 2) {
			if (!empty($order['oid'])) {
				$update['spreadfixed'] = 1;
...
function member_spread_bind()
{
	...
	else if ($group_become_child_limit == 2) {
		if (empty($_W['member']['is_mall_newmember'])) {
			return error(-1, '您不是新顾客，不能被推广');
		}
	...
	if (!empty($_W['member']['spread1']) && $_W['member']['spreadfixed'] == 1) {
		...
		if (!empty($spreads)) {
			...
			if (0 < $commission_spread1 || 0 < $commission_spread2) {
				...
				$data['data']['spread'] = array(
	'commission' => array('commission1_type' => $commission1_type, 'spread1_rate' => $spread1_rate . '%', 'spread1' => floatval($commission_spread1), 'commission2_type' => $commission2_type, 'spread2_rate' => $spread2_rate . '%', 'spread2' => floatval($commission_spread2), 'from_spread' => $_SESSION['from_spread_id'])
	);
			}
		}
	}
