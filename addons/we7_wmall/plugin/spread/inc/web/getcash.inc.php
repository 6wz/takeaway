...
if ($op == 'transfers') {
	...
	$params = array('partner_trade_no' => $log['trade_no'], 'openid' => $log['account']['openid'], 'check_name' => 'FORCE_CHECK', 're_user_name' => $log['account']['realname'], 'amount' => $log['final_fee'] * 100, 'desc' => $log['account']['realname'] . date('Y-m-d H:i', $log['addtime']) . '推广佣金提现申请');
	mload()->classs('wxpay');

	if ($log['channel_from'] == 'wxapp') {
		$params['openid'] = $log['account']['openid_wxapp'];

		if (empty($params['openid'])) {
			return error(-1, '模块版本为小程序版。未获取到推广员针对小程序的openid');
		}
		...
		if (empty($params['openid'])) {
			return error(-1, '模块版本为公众号版。未获取到推广员针对公众号的openid');
		}
...
if ($op == 'cancel') {
	...
	if ($_W['ispost']) {
		...
		$extra = array('trade_type' => 3, 'extra' => '', 'remark' => $remark);
