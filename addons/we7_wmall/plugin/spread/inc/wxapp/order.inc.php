...
$routers = array(
	'takeout' => array('table' => 'tiny_wmall_order', 'status_end' => 5, 'status_cancel' => 6, 'ordersn' => 'ordersn')
	);
$spread_types = array(
	array('type' => 'takeout', 'title' => '外卖订单')
	);
...
if ($op == 'index') {
	...
	$orders = pdo_fetchall('select id,spread1,spread2,' . $router['ordersn'] . ',paytime,spreadbalance,data,status,plateform_serve_fee from ' . tablename($router['table']) . $condition . ' order by id desc limit ' . ($page - 1) * $psize . ',' . $psize, $params);
	...
	$result = array(
		'records' => $orders,
		'config'  => array('spread_types' => $spread_types)
		);
