...
if ($op == 'index') {
	...
	$rule = array('start_fee' => $category['start_fee'], 'start_km' => $category['start_km'], 'pre_km_fee' => $category['pre_km_fee'], 'tip_min' => $category['tip_min'], 'tip_max' => $category['tip_max']);
...
if ($op == 'cart') {
	setcookie('errander_order', 0, -1000);
	$errander_order = array('goods_name' => trim($_GPC['goods_name']), 'goods_weight' => trim($_GPC['goods_weight']), 'goods_price' => trim($_GPC['goods_price']), 'note' => trim($_GPC['note']), 'delivery_tips' => trim($_GPC['delivery_tips']), 'is_anonymous' => trim($_GPC['is_anonymous']), 'start_address_id' => trim($_GPC['start_address_id']), 'end_address_id' => trim($_GPC['end_address_id']));
...
if ($op == 'addaddress') {
	...
	if ($_W['ispost']) {
		$data = array('uniacid' => $_W['uniacid'], 'uid' => $_W['member']['uid'], 'address' => trim($_GPC['address']), 'number' => trim($_GPC['number']), 'location_x' => floatval($_GPC['location_x']), 'location_y' => floatval($_GPC['location_y']), 'type' => 1);
...
if ($op == 'suggestion') {
	...
	$query = array('keywords' => $key, 'city' => '全国', 'output' => 'json', 'key' => '37bb6a3b1656ba7d7dc8946e7e26f39b');
