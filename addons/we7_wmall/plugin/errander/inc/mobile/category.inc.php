...
if ($op == 'index') {
	...
	$category['notice']['between_time'] = intval($category['notice']['between_time']);
	...
	$rule = array('type' => $category['type'], 'start_fee' => $category['start_fee'], 'start_km' => $category['start_km'], 'pre_km_fee' => $category['pre_km_fee'], 'weight_fee_status' => $category['weight_fee_status'], 'weight_fee' => $category['weight_fee'], 'tip_min' => $category['tip_min'], 'tip_max' => $category['tip_max'], 'multiaddress' => $category['multiaddress']);
	...
	if ($category['goods_value_status'] == 1) {
		$price_select = array(
...
if ($op == 'cart') {
	...
	$errander_order = array('goods_name' => trim($_GPC['goods_name']), 'goods_weight' => trim($_GPC['goods_weight']), 'goods_price' => trim($_GPC['goods_price']), 'note' => trim($_GPC['note']), 'delivery_tips' => trim($_GPC['delivery_tips']), 'is_anonymous' => trim($_GPC['is_anonymous']), 'start_address_id' => trim($_GPC['start_address_id']), 'end_address_id' => trim($_GPC['end_address_id']), 'multiaddress' => $_GPC['multiaddress'], 'thumbs' => $_GPC['thumbs']);
