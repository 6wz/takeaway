...
if ($op == 'serve_address') {
	$data = array('uniacid' => $_W['uniacid'], 'uid' => $_W['member']['uid'], 'name' => trim($_GPC['address']), 'address' => trim($_GPC['name']), 'location_x' => floatval($_GPC['location_x']), 'location_y' => floatval($_GPC['location_y']), 'number' => trim($_GPC['number']), 'type' => 2);
