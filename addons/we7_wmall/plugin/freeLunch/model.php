...
function freelunch_get()
{
    ...
    if( !empty($freelunch) ) 
    {
        $freelunch["thumb"] = tomedia($freelunch["thumb"]);
        $freelunch["plus_thumb"] = tomedia($freelunch["plus_thumb"]);
    }
    ...
    $freelunch["share"]["imgUrl"] = tomedia($freelunch["share"]["imgUrl"]);
    return $freelunch;
}
...
function freelunch_record_get($serial_sn, $type)
{
    ...
    $record["starttime_cn"] = date("Y-m-d H:i:s", $record["startime"]);
    return $record;
}
...
function freelunch_member_partaker($record_id, $uid = 0)
{
    ...
    else
    {
        ...
        {
            $partaker["addtime_cn"] = date("Y-m-d H:i:s", $partaker["addtime"]);
        }
    }
...
function freelunch_partaker_partake_status($uid, $id, $type)
{
    ...
    {
        $today_partake = pdo_fetchcolumn("select count(*) from " . tablename("tiny_wmall_freelunch_partaker") . " where uniacid = :uniacid and record_id = :record_id and uid = :uid and addtime > :time", array( ":uniacid" => $_W["uniacid"], ":record_id" => $id, ":uid" => $uid, ":time" => strtotime(date("Y-m-d")) ));
...
function freelunch_partaker_status_update($order_id, $type)
{
    ...
    if( $type == "pay" ) 
    {
        ...
        if( !$partaker_dosage ) 
        {
            ...
            if( $record["type"] == "plus" ) 
            {
                pdo_run("update ims_tiny_wmall_freelunch set plus_serial_sn = plus_serial_sn + 1 where id = " . $record["freelunch_id"]);
            }
            else
            {
                pdo_run("update ims_tiny_wmall_freelunch set serial_sn = serial_sn + 1 where id = " . $record["freelunch_id"]);
            }
