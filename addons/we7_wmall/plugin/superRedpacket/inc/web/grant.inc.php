...
if( $op == "list" ) 
{
    ...
    $params = array( ":uniacid" => $_W["uniacid"], ":type" => "superRedpacket" );
...
if( $op == "post" ) 
{
    ...
    if( $_W["ispost"] ) 
    {
        ...
        $grant_object = array( "total" => count($uid), "grant_success" => 0, "unissued_uid" => $uid );
        $menudata = array( "uniacid" => $_W["uniacid"], "name" => $data["name"], "type" => "superRedpacket", "data" => base64_encode(json_encode($data)), "grant_object" => iserializer($grant_object) );
        if( !empty($id) ) 
        {
    ...
    if( $_W["ispost"] ) 
    {
        ...
        foreach( $uids as $key => $uid ) 
        {
            ...
            foreach( $superRedpacket["data"]["redpackets"] as $redpacket ) 
            {
                ...
                $params = array( "title" => $redpacket["name"], "activity_id" => $superRedpacket["id"], "uid" => $uid, "channel" => "superRedpacket", "type" => "grant", "discount" => $redpacket["discount"], "condition" => $redpacket["condition"], "grant_days_effect" => $redpacket["grant_days_effect"], "days_limit" => $redpacket["use_days_limit"], "is_show" => 0, "scene" => $redpacket["scene"], "order_type_limit" => $redpacket["order_type_limit"] );
                $times_limit = array(  );
                if( !empty($redpacket["times"]) ) 
                {
            ...
            if( $superRedpacket["data"]["customer"]["template_notice"] == 1 ) 
            {
                ...
                if( !empty($openid) ) 
                {
                    ...
                    $params = array( "first" => "您在" . $config["mall"]["title"] . "的账户有" . $num . "个代金券到账", "keyword1" => "账户代金券", "keyword2" => (string) $discount . "元", "keyword3" => date("Y-m-d H:i", TIMESTAMP), "keyword4" => "账户有新的代金券到账", "remark" => implode("\n", array( "感谢您对" . $config["mall"]["title"] . "平台的支持与厚爱。点击查看红包>>" )) );
