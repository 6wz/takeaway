...
', $remark));
		...
		if ($order_channel == 'wxapp') {
			$send = array(
				'keyword1' => array('value' => '#' . $order['id'], 'color' => '#ff510'),
				'keyword2' => array('value' => $order['order_type_cn'], 'color' => '#ff510'),
				'keyword3' => array('value' => '已生效', 'color' => '#ff510'),
				'keyword4' => array('value' => $member['realname'], 'color' => '#ff510'),
				'keyword5' => array('value' => $member['mobile'], 'color' => '#ff510'),
...
{
	global $_W;
	$redpacket = pdo_get('tiny_wmall_svip_redpacket', array('uniacid' => $_W['uniacid'], 'agentid' => $_W['agentid'], 'id' => $id));
...
{
	$data = array(
		array('text' => '已结束或已撤销', 'css' => 'label label-default'),
...
{
	$types = array(
		'oneChargeFee'   => array('type' => 'oneChargeFee', 'title' => '单笔充值满额', 'css' => 'label label-success', 'achieve_text' => '去充值', 'achieve_link' => '/pages/member/recharge'),
		'oneOrderFee'    => array('type' => 'oneOrderFee', 'title' => '单笔外卖满额', 'css' => 'label label-info', 'achieve_text' => '去完成', 'achieve_link' => '/pages/home/index'),
		'oneErranderFee' => array('type' => 'oneErranderFee', 'title' => '单笔跑腿满额', 'css' => 'label label-warning', 'achieve_text' => '去完成', 'achieve_link' => '/pages/paotui/guide')
		);
...
{
	$status_all = array(
		array('css' => 'label label-danger', 'text' => '已结束'),
		array('css' => 'label label-success', 'text' => '进行中'),
		array('css' => 'label label-warning', 'text' => '未开始')
		);
...
{
	$status_all = array(
		1 => array('css' => 'label label-warning', 'text' => '进行中'),
		2 => array('css' => 'label label-success', 'text' => '已完成'),
		3 => array('css' => 'label label-danger', 'text' => '未完成')
		);
			...
			if ($task_takepart_type == 3) {
				if (!empty($is_exist)) {
					...
					if ($is_exist['status'] == 2 || $is_exist['status'] == 3) {
						...
						else if ($data['takepart_repeat_time'] == 'week') {
...
function svip_task_finish_check($uid, $task_type, $order = array())
{
	...
	if (in_array($task_type, array('oneOrderFee', 'oneChargeFee', 'oneErranderFee'))) {
		if ($order['final_fee'] < $task_data['condition']) {
...
function svip_store_useful_redpacket_get($sid, $discount_max = 0, $extra = array())
{
	...
	$condition = ' where a.uniacid = :uniacid and a.agentid =:agentid and a.status = :status and a.sid = :sid and a.discount <= :discount and a.condition <= :discount ';
	$params = array(':uniacid' => $_W['uniacid'], ':agentid' => $_W['agentid'], ':status' => 1, ':sid' => $sid, ':discount' => $discount_max);
	...
	if (!empty($store_redpacket)) {
		...
		if (0 < $num_taking) {
			...
			$store_redpacket['desc'] = $store_redpacket['store_title'] . '门店专享';
...
{
	global $_W;
	$condition = ' where uniacid = :uniacid and agentid = :agentid and status = :status and sid = :sid and discount < :discount and `condition` <= :discount ';
	$params = array(':uniacid' => $_W['uniacid'], ':agentid' => $_W['agentid'], ':status' => 1, ':sid' => 0, ':discount' => $discount_max);
...
function svip_before_timeout_notice()
{
	...
	if (!empty($svip_members)) {
		...
		foreach ($svip_members as $item) {
			$params = array('first' => $item['nickname'] . '，您的【' . $config['title'] . '】会员即将到期，到期后不再享有会员特权，记得续费哦~', 'keyword1' => '会员变动', 'keyword2' => '', 'keyword3' => date('Y-m-d H:i', TIMESTAMP), 'keyword4' => '您的会员即将到期', 'remark' => implode('
...
{
	$data = array(
		1 => array('text' => '待兑换', 'css' => 'label label-danger'),
		2 => array('text' => '已兑换', 'css' => 'label label-success'),
		3 => array('text' => '已过期', 'css' => 'label label-default')
		);
	...
	if (!empty($codes)) {
		...
		foreach ($codes as &$code) {
			...
			if ($code['status'] == 2) {
				...
				$code['exchangetime_cn'] = '兑换码未被使用，已过期';
			}
			else {
