...
if ($op == 'post') {
	...
	if ($_W['ispost']) {
		$data = array('sid' => intval($_GPC['sid']), 'uniacid' => $_W['uniacid'], 'title' => trim($_GPC['title']), 'number' => trim($_GPC['number']), 'price' => floatval($_GPC['price']), 'old_price' => floatval($_GPC['old_price']), 'unitname' => trim($_GPC['unitname']), 'total' => intval($_GPC['total']), 'total_warning' => intval($_GPC['total_warning']), 'total_update_type' => intval($_GPC['total_update_type']), 'sailed' => intval($_GPC['sailed']), 'status' => intval($_GPC['status']), 'cid' => intval($_GPC['category']['parentid']), 'child_id' => intval($_GPC['category']['childid']), 'box_price' => floatval($_GPC['box_price']), 'thumb' => trim($_GPC['thumb']), 'label' => trim($_GPC['label']), 'displayorder' => intval($_GPC['displayorder']), 'content' => trim($_GPC['content']), 'description' => htmlspecialchars_decode($_GPC['description']), 'is_options' => intval($_GPC['is_options']), 'is_hot' => intval($_GPC['is_hot']), 'print_label' => intval($_GPC['print_label']), 'is_showtime' => intval($_GPC['is_showtime']));
...
if ($op == 'index') {
	...
	$lists = pdo_fetchall('SELECT * FROM ' . tablename('tiny_wmall_goods') . ($condition . ' ORDER BY id desc LIMIT ') . ($pindex - 1) * $psize . ',' . $psize, $params);
	$wmStore = pdo_fetchall('select id,title from ' . tablename('tiny_wmall_store') . ' where uniacid = :uniacid order by displayorder desc, id asc', array(':uniacid' => $_W['uniacid']), 'id');
		...
		if (!empty($category_ids)) {
			foreach ($category_ids as $cid) {
				...
				foreach ($goods as $value) {
					$useful_keys = array('uniacid', 'title', 'number', 'price', 'old_price', 'box_price', 'is_options', 'unitname', 'total', 'status', 'is_hot', 'thumb', 'slides', 'label', 'displayorder', 'content', 'description', 'attrs');
