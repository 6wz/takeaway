...
{
	global $_W;
	$menu = pdo_fetchall('SELECT * FROM ' . tablename('tiny_wmall_diypage_menu') . ' WHERE uniacid = :uniacid and version = :version', array(':uniacid' => $_W['uniacid'], ':agentid' => $_W['agentid'], ':version' => $version));
	return $menu;
}
					...
					foreach ($goodsids as $goodsid) {
						foreach ($goods as $good) {
							if ($good['id'] == $goodsid) {
								...
								$childid = 'C' . $childid;
								$item['data'][$childid] = array('goods_id' => $good['id'], 'sid' => $good['sid'], 'store_title' => $good['store_title'], 'thumb' => $good['thumb'], 'title' => $good['title'], 'price' => $good['price'], 'old_price' => $good['old_price'], 'sailed' => $good['sailed'], 'total' => $good['total'] != -1 ? $good['total'] : '无限', 'discount' => $good['old_price'] == 0 ? 0 : round($good['price'] / $good['old_price'] * 10, 1), 'comment_good_percent' => $good['comment_total'] == 0 ? 0 : round($good['comment_good'] / $good['comment_total'] * 100, 2) . '%');
							}
						}
					}
				}
		...
		if ($item['params']['goodsdata'] == '1') {
			...
			if (!empty($goods)) {
				...
				foreach ($goods as &$good) {
					$childid = rand(1000000000, 9999999999);
					$childid = 'C' . $childid;
					$item['data'][$childid] = array('goods_id' => $good['id'], 'sid' => $good['sid'], 'store_title' => $stores[$good['sid']]['store_title'], 'thumb' => $good['thumb'], 'title' => $good['title'], 'price' => $good['discount_price'], 'old_price' => $good['old_price'], 'sailed' => $good['sailed'], 'total' => $good['discount_available_total'] != -1 ? $good['discount_available_total'] : '无限', 'discount' => $good['old_price'] == 0 ? 0 : round($good['discount_price'] / $good['old_price'] * 10, 1), 'comment_good_percent' => $good['comment_total'] == 0 ? 0 : round($good['comment_good'] / $good['comment_total'] * 100, 2) . '%');
				}
			}
		}
	}
...
function get_diypage_waimai_store($item, $mobile = false)
{
	...
	if ($item['params']['storedata'] == '0') {
	...
	else if ($item['params']['storedata'] == '2') {
		...
		$stores = pdo_fetchall('select id, title, logo, delivery_free_price, score,is_rest,delivery_time,sailed,label from ' . tablename('tiny_wmall_store') . $condition . (' order by displayorder desc limit ' . $limit), $params);
	}
	else {
		...
		foreach ($stores as &$row) {
			...
			if (!empty($activitys['items'])) {
				foreach ($activitys['items'] as $avtivity_item) {
					...
					$childid = rand(1000000000, 9999999999);
					$childid = 'C' . $childid;
					$row['activity']['items'][$childid] = array('type' => $avtivity_item['type'], 'title' => $avtivity_item['title']);
				}
...
{
	global $_W;
	$share = array('title' => $page['data']['page']['title'], 'imgUrl' => tomedia($page['data']['page']['thumb']), 'desc' => $page['data']['page']['desc'], 'link' => imurl('diypage/diy/index', array('id' => $page['id']), true));
	return $share;
}
