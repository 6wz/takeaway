...
if ($op == "list") {
    ...
    if ($op == "post") {
        ...
        if ($_W["ispost"]) {
            $data = $_GPC["data"];
            $diydata = array("uniacid" => $_W["uniacid"], "name" => $data["page"]["name"], "type" => $data["page"]["type"], "data" => base64_encode(json_encode($data)), "updatetime" => TIMESTAMP, "version" => 2);
            ...
            if ($op == "copy") {
                ...
                if ($op == "diypageagent") {
                    ...
                    if ($_W["ispost"] && $_GPC["set"] == 1) {
                        $diypageid = explode(",", $_GPC["id"]);
                        $agentid = intval($_GPC["agentid"]);
                        if (0 < $agentid) {
                            foreach ($diypageid as $value) {
                                pdo_update("tiny_wmall_diypage", array("agentid" => $agentid), array("uniacid" => $_W["uniacid"], "id" => $value));
                            }
                        }
                        imessage(error(0, '批量操作修改成功'), iurl('diypage/diyPage/list'), 'ajax');
                    }
                    include itemplate('vue/op');
                }
            }
        }
    }
