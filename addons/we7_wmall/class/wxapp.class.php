...
class Wxapp extends TyAccount
{
    public function __construct($account = "")
    {
        ...
        if( !is_array($account) ) 
        {
            $account = get_plugin_config("wxapp.basic");
        }
    ...
    public function sendTplNotice($touser, $template_id, $postdata, $url, $form_id, $color = "#FF683F")
    {
        if( empty($this->account["secret"]) || empty($this->account["key"]) ) 
        {
            return error(-1, "请完善小程序key和secret");
        }
        ...
        if( empty($form_id) ) 
        {
            ...
            {
                return error(-1, "没有可用的form_id用于发送微信小程序模版消息");
            }
        ...
        $data = array( "touser" => $touser, "template_id" => trim($template_id), "page" => trim($url), "color" => trim($color), "data" => $postdata, "form_id" => $form_id, "emphasis_keyword" => "keyword1.DATA" );
        ...
        {
            return error(-1, "访问微信接口错误, 错误代码b: " . $result["errcode"] . ", 错误信息: " . $result["errmsg"] . ",信息详情：" . $this->errorCode($result["errcode"]));
        }
    ...
    public function getDailyVisitTrend()
    {
        ...
        $data = array( "begin_date" => date("Y-m-d", strtotime("-1 days")), "end_date" => date("Y-m-d", strtotime("-1 days")) );
        $response = $this->requestApi($url, json_encode($data));
        if( is_error($response) ) 
        {
            return $response;
