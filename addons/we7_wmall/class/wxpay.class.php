...
class WxPay
{
	...
	public function mktQueryBank($params)
	{
		...
		$msg = array(
			'SUCCESS'    => array('errno' => 0, 'text' => '该提现已成功到账', 'toaccount_status' => 2),
			'PROCESSING' => array('errno' => 0, 'text' => '该提现正在处理中，请耐心等待', 'toaccount_status' => 1),
			'FAILED'     => array('errno' => -1, 'text' => '该提现到银行卡已失败', 'toaccount_status' => 3),
			'BANK_FAIL'  => array('errno' => -1, 'text' => '该提现申请银行退票', 'toaccount_status' => 3)
			);
		return error(0, array("status" => $result['status'], 'errno' => $msg[$result['status']]['errno'], 'toaccount_status' => $msg[$result['status']]['toaccount_status'], 'msg' => $msg[$result['status']]['text']));
	}
	...
	public function toRSA($data, $force = true)
	{
		...
		if (empty($public_key)) {
			...
			if ($result['result_code'] == 'SUCCESS') {
				...
				$status = ifile_put_contents('cert/' . $name . '/public_key.pem', $public_key);
				set_system_config("payment_rsa", $name);
			}
			 else {
				return error(-1, $result['err_code'] . '：' . $result['err_code_des']);
			}
	...
	{
		$wechat_status = array(
			'SUCCESS'    => array('text' => '成功', 'value' => 3),
			'FAIL'       => array('text' => '失败', 'value' => 4),
			'PROCESSING' => array('text' => '处理中', 'value' => 2),
			'NOTSURE'    => array('text' => '未确定，需要商户原退款单号重新发起', 'value' => 5)
			);
	...
	{
		$bank_list = array(
			1002 => array('id' => '1002', 'title' => '工商银行'),
			1005 => array('id' => '1005', 'title' => '农业银行'),
			1026 => array('id' => '1026', 'title' => '中国银行'),
			1003 => array('id' => '1003', 'title' => '建设银行'),
			1001 => array('id' => '1001', 'title' => '招商银行'),
			1066 => array('id' => '1066', 'title' => '邮储银行'),
			1020 => array('id' => '1020', 'title' => '交通银行'),
			1004 => array('id' => '1004', 'title' => '浦发银行'),
			1006 => array('id' => '1006', 'title' => '民生银行'),
			1009 => array('id' => '1009', 'title' => '兴业银行'),
			1010 => array('id' => '1010', 'title' => '平安银行'),
			1021 => array('id' => '1021', 'title' => '中信银行'),
			1025 => array('id' => '1025', 'title' => '华夏银行'),
			1027 => array('id' => '1027', 'title' => '广发银行'),
			1022 => array('id' => '1022', 'title' => '光大银行'),
			1032 => array('id' => '1032', 'title' => '北京银行'),
			1056 => array('id' => '1056', 'title' => '宁波银行')
			);
		return $bank_list;
	}
}
