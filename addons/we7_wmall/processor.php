...
class We7_wmallModuleProcessor extends WeModuleProcessor
{
	public function respond()
	{
		...
		if (\in_array($row['type'], array('store', 'assign', 'table'))) {
			...
			$news = array(
				array('title' => '您的好友(' . $member['nickname'] . ')向您推荐了' . $config_mall['title'], 'description' => $config_share['desc'], 'picurl' => tomedia($config_mall['logo']), 'url' => ivurl('pages/home/index', array('code' => $row['extra']['uid']), true))
				);
			return $this->respNews($news);
		}
