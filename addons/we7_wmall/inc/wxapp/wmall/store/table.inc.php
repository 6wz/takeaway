                    ...
                    else
                    {
                        ...
                        else
                        {
                            if( $ta == "submit" ) 
                            {
                                ...
                                $table_id = intval($_GPC["table_id"]);
                                $order = array( "uniacid" => $_W["uniacid"], "acid" => $_W["acid"], "sid" => $sid, "uid" => $_W["member"]["uid"], "ordersn" => date("YmdHis") . random(6, true), "serial_sn" => store_order_serial_sn($sid), "code" => random(4, true), "order_type" => 3, "openid" => $_W["openid"], "mobile" => "", "username" => trim($condition["username"]), "person_num" => intval($condition["person_num"]), "table_id" => $table_id, "sex" => "", "address" => "", "location_x" => "", "location_y" => "", "delivery_day" => "", "delivery_time" => "", "delivery_fee" => 0, "pack_fee" => 0, "pay_type" => "", "num" => $cart["num"], "price" => $cart["price"], "serve_fee" => $serve_fee, "total_fee" => $cart["price"] + $serve_fee, "discount_fee" => $activityed["total"], "store_discount_fee" => $activityed["store_discount_fee"], "plateform_discount_fee" => $activityed["plateform_discount_fee"], "final_fee" => ($cart["price"] + $serve_fee) - $activityed["total"], "status" => 1, "is_comment" => 0, "invoice" => $invoice, "addtime" => TIMESTAMP, "data" => array( "cart" => $cart["original_data"] ), "note" => trim($condition["note"]) );
