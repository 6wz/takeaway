...
if( $ta == "index" ) 
{
        ...
        if( $store["data"]["zhunshibao"]["fee_type"] == 2 ) 
        {
            ...
            if( !empty($store["data"]["zhunshibao"]["rule"]) ) 
            {
                foreach( $store["data"]["zhunshibao"]["rule"] as $val ) 
                {
                    ...
                    $rule_cn .= "延误" . $val["time"] . "分钟,赔" . $compensate_fee . "元,";
                }
            }
    ...
    if( check_plugin_perm("huangou") ) 
    {
        ...
        if( $store["data"]["huangou"]["status"] == 1 ) 
        {
            ...
            $result["huangou"] = array( "goods" => $huangou_goods, "message" => $message_huangou, "cart_message" => $huangou_cart_message, "price_limit" => $huangou["price_limit"], "can_buy_huangou_goods" => ($huangou["price_limit"] <= $order["total_fee_show"] ? 1 : 0) );
        }
...
else
{
    if( $ta == "submit" ) 
    {
        ...
        if( $params["order_type"] != 2 ) 
        {
            ...
            if( !$reach_sendprice ) 
            {
                imessage(error(1000, "未达到商户起送价"), "", "ajax");
            }
        ...
        if( 0 < $calculate["zhunshibao_price"] ) 
        {
