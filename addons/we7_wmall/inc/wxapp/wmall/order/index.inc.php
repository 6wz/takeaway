...
else
{
    if( $ta == "detail" ) 
    {
        ...
        $show_location = 0;
        if( $order["status"] == 4 && in_array($order["delivery_handle_type"], array( "app", "wxapp" )) ) 
        {
            $show_location = 1;
        }
                                        ...
                                        else
                                        {
                                            if( $ta == "update_address" ) 
                                            {
                                                ...
                                                if( $_W["ispost"] ) 
                                                {
                                                    ...
                                                    {
                                                        imessage(error(-1, "新地址与原地址的距离超过" . $config["newaddress_distance"] . "km, 无法修改"), "", "ajax");
                                                    }
                                            ...
                                            else
                                            {
                                                if( $ta == "zhunshibao" ) 
                                                {
                                                    ...
                                                    $order["endtime_cn"] = date("Y-m-d H:i", $order["endtime"]);
                                                    $order["prredict_time_cn"] = date("Y-m-d H:i", $prredict_time);
