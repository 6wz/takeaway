    ...
    if( !empty($orders) ) 
    {
        ...
        foreach( $orders as &$da ) 
        {
            ...
            if( $da["is_reserve"] == 1 && $da["status"] == 1 ) 
            {
                ...
                $da["handle_tip"] = "本单属于预订单，顾客选择送达时间为" . date("Y-m-d H:i", $da["deliverytime"]) . "当前距离送达时间还有" . $time_difference . "， 现在接单吗？";
            }
...
else
{
    if( $ta == "detail" ) 
    {
        ...
        if( $order["is_reserve"] == 1 && $order["status"] == 1 ) 
        {
            ...
            $time_difference = $time_difference - $time_difference % 60;
            $time_difference = transform_time($time_difference);
            $order["handle_tip"] = "本单属于预订单，顾客选择送达时间为" . date("Y-m-d H:i", $order["deliverytime"]) . "当前距离送达时间还有" . $time_difference . ", 现在接单吗？";
        }
