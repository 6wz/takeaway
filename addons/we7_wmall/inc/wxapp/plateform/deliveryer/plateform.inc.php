...
if ($ta == 'change_status') {
	...
	if ($type == 'work_status') {
	 ...
	 else if (in_array($type, array('accept_wechat_notice', 'accept_voice_notice'))) {
...
if ($ta == 'comision') {
	...
	if ($_W['ispost']) {
		...
		$update = array('fee_delivery' => iserializer($fee_delivery), 'fee_getcash' => iserializer($fee_getcash));
