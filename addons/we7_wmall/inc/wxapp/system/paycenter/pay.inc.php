...
$title = $router["title"];
//支付订单页面返回的数据
$data = array( "title" => $title, "logo" => tomedia($logo), "fee" => $record["fee"],"test" => "111" );
...
if( $_GPC["type"] ) 
{
    ...
    mload()->model("payment");
    $result['on1']=(int) $_W["we7_wmall"]["config"]["payment"]['suppay']['on'];
	$result['on2']=(int) $_W["we7_wmall"]["config"]["payment"]['epayment']['on'];
	$result['on3']=(int) $_W["we7_wmall"]["config"]["payment"]['stripe']['on'];
	$result['on4']=(int) $_W["we7_wmall"]["config"]["payment"]['attract']['on'];
...
if( $pay_type && !$_GPC["type"] && in_array($pay_type, array_keys($payment)) ) 
{
    ...
    if($pay_type == "card" ){
	   ...
	   if(empty($stripe_info)){
	      $ret["code"] =1;
		  $ret["msg"] ="您目前还未填写银行卡信息，请补充完整银行卡信息";
		  imessage(error(0,$ret), "", "ajax");
		  die();
	   }
	   $stripe_obj=new stripe($payconfig['sk']);
	   $return=$stripe_obj->Charge($stripe_info,$log,$_W["member"]["uid"],$payconfig['bz']);
    ...
    if($pay_type == "supay" ){
	    mload()->model("payment");
	   $payconfig=$_W["we7_wmall"]["config"]["payment"]['suppay'];
	   load()->library('superpayService.class');
	   $appid= $payconfig['appid'];
	   $setting['appid']= $appid;
	   $setting['mchId']=$payconfig['mchId'];
	   $setting['md5Key']=$payconfig['md5Key'];
	   $setting['currency']=$payconfig['bz'];
	   $supay=new superpayService($setting,$appid);
	   $openid=$log['openid'];
	   $price=$log['fee'];
	   $order_no=$log['tid'];
	   $notify_url=$_W["siteroot"]."suppay.php";
	   $goods_names='supay支付商品';
	   $result=$supay->miniApp($order_no,$openid,$price,$notify_url,$goods_names);
	   $ret["wheel_url"] = $wheel_url;
	   $ret["pay_type"] ='supay';
	   $ret["paySign"] =$result['paySign'];
	   $ret["nonceStr"] =$result['nonceStr'];
	   $ret["package"] =$result['package'];
	   $ret["signType"] =$result['signType'];
	   $ret["timeStamp"] =$result['timeStamp'];
	   imessage(error(0,$ret), "", "ajax");
	}	
	
	
	
    if($pay_type == "attractpay" ){
	    mload()->model("payment");
	   $payconfig=$_W["we7_wmall"]["config"]["payment"]['attract'];
	   load()->library('AttractPay.class');
	   $appid= $payconfig['appid'];
	   $mch_store_id=$payconfig['mch_store_id'];
	   $authentication_code=$payconfig['authentication_code'];
	   $openid=$log['openid'];
	   $price=$log['fee'];
	   $order_no=$log['tid'];
	   $goods_names='attractpay支付商品';
	   $bz=$payconfig['bz'];
	   $notify_url=$_W["siteroot"]."attractpay.php";
	   $accpay_obj=new AttractPay($mch_store_id,$authentication_code);
	   $result=$accpay_obj->createPay($order_no,$openid,$appid,$price,$notify_url,$goods_names,$bz);
	   $ret["wheel_url"] = $wheel_url;
	   $ret["pay_type"] ='attractpay';
	   $ret["paySign"] =$result['paySign'];
	   $ret["nonceStr"] =$result['nonceStr'];
	   $ret["package"] =$result['package'];
	   $ret["signType"] =$result['signType'];
    ...
    if($pay_type == "epayment" ){
	  mload()->model("payment");
	   $payconfig=$_W["we7_wmall"]["config"]["payment"]['epayment'];
	   load()->library('Epayment.class');
	   $appid=$payconfig['appid'];
	   $mch_store_id=$payconfig['mch_store_id'];
	   $authentication_code=$payconfig['authentication_code'];
	   $epayment=new Epayment($mch_store_id,$authentication_code);
	   $openid=$log['openid'];
	   $price=$log['fee']*100;
	   $order_no=$log['tid'];
	   $notify_url=$_W["siteroot"]."epayment.php";
	   $goods_names='empayment支付商品';
	   $result=$epayment->createPay($order_no,$openid,$appid,$price,$notify_url,$goods_names,$payconfig['bz']);	   
	   $ret["wheel_url"] = $wheel_url;
	   $ret["pay_type"] ='epayment';
	   $ret["paySign"] =$result['paySign'];
	   $ret["nonceStr"] =$result['nonceStr'];
	   $ret["package"] =$result['package'];
	   $ret["signType"] =$result['signType'];
