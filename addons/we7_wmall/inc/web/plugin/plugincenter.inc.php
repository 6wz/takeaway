...
if ($op == 'buy') {
	...
	$config = get_plugin_config('plugincenter');
	...
	if ($_W['ispost']) {
		...
		$insert = array('uniacid' => $_W['uniacid'], 'pluginname' => $detail['pluginname'], 'pluginid' => $detail['pluginid'], 'month' => $meal['month'], 'final_fee' => $meal['price'], 'order_sn' => date('YmdHis') . random(6, true), 'addtime' => TIMESTAMP, 'pay_type' => 'alipay', 'data' => iserializer(array(
	'meal' => array('id' => $id, 'type' => $type, 'title' => $detail['title'], 'thumb' => $detail['thumb'])
	)));
		pdo_insert("tiny_wmall_plugincenter_order", $insert);
		$order_id = pdo_insertid();
		mload()->model("payment");
		$params = pc_pay_prep(array('id' => $order_id, 'order_type' => 'plugincenter'));
		$alipay_config = get_plugin_config('plugincenter.pay_type.alipay');
