...
if( $op == "post" )
{
    ...
    if( $_W["ispost"] )
    {
        $data = array( "uniacid" => $_W["uniacid"], "title" => trim($_GPC["title"]), "title_th" => trim($_GPC["title_th"]), "logo" => trim($_GPC["logo"]), "telephone" => trim($_GPC["telephone"]), "content" => trim($_GPC["content"]),"content_th" => trim($_GPC["content_th"]), "address" => trim($_GPC["address"]),"address_th" => trim($_GPC["address_th"]), "displayorder" => intval($_GPC["displayorder"]), "delivery_mode" => ($config_store["delivery"]["delivery_mode"] ? $config_store["delivery"]["delivery_mode"] : 1), "delivery_fee_mode" => 1, "delivery_price" => $config_store["delivery"]["delivery_fee"], "business_hours" => iserializer(array(  )), "addtime" => TIMESTAMP, "push_token" => random(32), "self_audit_comment" => intval($config_store["extra"]["self_audit_comment"]) );
...
if( $op == "batch" )
{
    ...
    {
        pdo_update("tiny_wmall_store", array( $type => $value ), array( "uniacid" => $_W["uniacid"] ));
    }
