...
if ($op == 'add') {
	$_W['page']['title'] = '活动创建';
	$all_activity = array(
		'mallNewMember'       => array('title' => '平台新用户', 'key' => 'mallNewMember', 'label' => 'label-danger'),
		'newMember'           => array('title' => '门店新用户', 'key' => 'newMember', 'label' => 'label-danger'),
		'discount'            => array('title' => '满减优惠', 'key' => 'discount', 'label' => 'label-danger'),
		'cashGrant'           => array('title' => '下单返现', 'key' => 'cashGrant', 'label' => 'label-success'),
		'grant'               => array('title' => '下单满赠', 'key' => 'grant', 'label' => 'label-success'),
		'selfDelivery'        => array('title' => '自提优惠', 'key' => 'selfDelivery', 'label' => 'label-warning'),
		'deliveryFeeDiscount' => array('title' => '满减配送费', 'key' => 'deliveryFeeDiscount', 'label' => 'label-warning'),
					...
					if ($condition && $back) {
						...
						if (!(empty($_W['ismanager']))) {
							...
							if ($back < $data[$condition]['agent_charge']) {
							 ...
							 else if ($back < $data[$condition]['plateform_charge']) {
								$data[$condition]['plateform_charge'] = $back;
								$data[$condition]['agent_charge'] = 0;
								$data[$condition]['store_charge'] = 0;
							}
							 else if ($back < ($data[$condition]['plateform_charge'] + $data[$condition]['agent_charge'])) {
						...
						if ($type == 'discount') {
						 ...
						 else if ($type == 'deliveryFeeDiscount') {
							$title[] = '满' . $condition . '元配送费减' . $back . '元';
						}
						 else if ($type == 'cashGrant') {
							$title[] = '满' . $condition . '元返' . $back . '元';
						}
						 else if ($type == 'selfPickup') {
			...
			if (($type == 'newMember') || ($type == 'mallNewMember')) {
				...
				if (!(empty($_W['ismanager']))) {
					...
					if ($back < $data['agent_charge']) {
					 ...
					 else if ($back < $data['plateform_charge']) {
						$data['plateform_charge'] = $back;
						$data['agent_charge'] = 0;
						$data['store_charge'] = 0;
					}
					 else if ($back < ($data['plateform_charge'] + $data['agent_charge'])) {
