...
if( $op == "index" ) 
{
    ...
    $storeParams = array( ":uniacid" => $_W["uniacid"] );
    ...
    $deliveryerParams = array( ":uniacid" => $_W["uniacid"] );
    ...
    $deliveryer["total_work_deliveryer"] = intval(pdo_fetchcolumn("select count(*) from " . tablename("tiny_wmall_deliveryer") . (string) $deliveryerCondition . " and status = 1", $deliveryerParams));
    $deliveryer["total_rest_deliveryer"] = intval(pdo_fetchcolumn("select count(*) from " . tablename("tiny_wmall_deliveryer") . (string) $deliveryerCondition . " and status = 0", $deliveryerParams));
