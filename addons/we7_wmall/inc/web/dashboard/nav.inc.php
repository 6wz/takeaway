        ...
        if( $config_mall["store_use_child_category"] == 1 ) 
        {
            ...
            if( !empty($da["child"]) ) 
            {
                foreach( $da["child"] as &$val ) 
                {
                    ...
                    $val["is_sys"] = 0;
                    if( empty($val["link"]) && empty($val["wxapp_link"]) ) 
                    {
                        $val["is_sys"] = 1;
...
if( $op == "post" ) 
{
    ...
    if( $_W["ispost"] ) 
    {
        ...
        $data = array( "uniacid" => $_W["uniacid"], "title" => $title, "parentid" => intval($_GPC["parentid"]), "thumb" => trim($_GPC["thumb"]), "link" => trim($_GPC["link"]), "displayorder" => intval($_GPC["displayorder"]), "nav_status" => intval($_GPC["nav_status"]), "slide_status" => intval($_GPC["slide_status"]), "nav" => iserializer($nav), "slide" => iserializer($slide), "wxapp_link" => trim($_GPC["wxapp_link"]) );
