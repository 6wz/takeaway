...
if ($ta == 'index') {
	...
	if ($_W['isajax']) {
		...
		$chart = array(
			'stat'   => $stat,
			'fields' => array('total_success_order', 'total_fee', 'store_final_fee', 'store_discount_fee', 'plateform_discount_fee', 'agent_discount_fee', 'plateform_serve_fee', 'plateform_delivery_fee', 'total_cancel_order', 'total_cancel_fee'),
			'titles' => array('有效订单量', '营业总额', '总收入', '商家补贴', '平台补贴', '代理补贴', '平台服务费', '平台配送费', '无效订单量', '损失营业额')
			);
		$i = $starttime;
	...
	while ($starttime <= $i) {
		if (empty($records_temp[$i])) {
			$records[] = array('stat_day' => $i, 'total_success_order' => 0, 'final_fee' => 0, 'store_final_fee' => 0, 'plateform_discount_fee' => 0, 'store_discount_fee' => 0, 'agent_discount_fee' => 0, 'plateform_serve_fee' => 0, 'plateform_delivery_fee' => 0);
		}
		 else {
