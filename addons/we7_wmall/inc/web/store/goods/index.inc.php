...
if( $ta == "post" )
{
    ...
    if( $id )
    {
        ...
        $item["attrs_th"] = iunserializer($item["attrs_th"]);
        if( !empty($item["attrs_th"]) )
        {
            foreach( $item["attrs_th"] as &$val )
            {
                $val["label"] = implode(",", $val["label"]);
            }
        }
    ...
    if( $_W["ispost"] )
    {
        ...
        $data = array( "sid" => $sid, "uniacid" => $_W["uniacid"], "title" => trim($_GPC["title"]), "title_th" => trim($_GPC["title_th"]), "number" => trim($_GPC["number"]), "type" => intval($_GPC["type"]), "price" => floatval($_GPC["price"]), "old_price" => floatval($_GPC["old_price"]), "ts_price" => floatval($_GPC["ts_price"]), "unitname" => trim($_GPC["unitname"]),"unitname_th" => trim($_GPC["unitname_th"]), "unitnum" => (1 < intval($_GPC["unitnum"]) ? intval($_GPC["unitnum"]) : 1), "total" => intval($_GPC["total"]), "total_warning" => intval($_GPC["total_warning"]), "total_update_type" => intval($_GPC["total_update_type"]), "sailed" => intval($_GPC["sailed"]), "status" => intval($_GPC["status"]), "cid" => intval($_GPC["category"]["parentid"]), "child_id" => intval($_GPC["category"]["childid"]), "box_price" => floatval($_GPC["box_price"]), "thumb" => trim($_GPC["thumb"]), "label" => trim($_GPC["label"]),"label_th" => trim($_GPC["label_th"]), "displayorder" => intval($_GPC["displayorder"]), "content" => trim($_GPC["content"]),"content_th" => trim($_GPC["content_th"]), "description" => htmlspecialchars_decode($_GPC["description"]), "description_th" => htmlspecialchars_decode($_GPC["description_th"]), "is_options" => intval($_GPC["is_options"]), "is_hot" => intval($_GPC["is_hot"]), "print_label" => intval($_GPC["print_label"]), "is_showtime" => intval($_GPC["is_showtime"]) );
        $data["svip_status"] = 0;
        if( $svip_perm )
        {
        ...
        if( $data["is_options"] == 1 )
        {
            ...
            {
                $val = trim($val);
                $val_th = trim($_GPC["options"]["name_th"][$key]);
        ...
        $data["attrs_th"] = array(  );
        if( !empty($_GPC["attrs_th"]) )
        {
            foreach( $_GPC["attrs_th"]["name"] as $key => $row )
            {
                $row = trim($row);
                if( empty($row) )
                {
                ...
                $labels = $_GPC["attrs_th"]["label"][$key];
                $labels = array_filter(explode(",", str_replace("，", ",", $labels)), trim);
                if( empty($labels) )
                {
                    continue;
                ...
                $data["attrs_th"][] = array( "name" => $row, "label" => $labels );
            }
        }

        $data["attrs_th"] = iserializer($data["attrs_th"]);
...
else
{
    if( $ta == "svip_price" )
    {
        if( $_W["ispost"] )
        {
            ...
            {
                imessage(error(-1, "请设置0-10之间的折扣数值"), referer(), "error");
            }
