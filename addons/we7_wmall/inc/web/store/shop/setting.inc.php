...
if( $ta == "post" )
{
    ...
    if( $_W["ispost"] )
    {
        ...
        if( !empty($_GPC["remind_reply_th"]) )
        {
            $remind_reply_th = array(  );
            foreach( $_GPC["remind_reply_th"] as $reply_th )
            {
                $reply_th = trim($reply_th);
                if( empty($reply_th) )
                {
                    continue;
                }

                $remind_reply_th[] = $reply_th;
            }
            $data["remind_reply_th"] = iserializer($remind_reply_th);
        }
        else
        {
            $data["remind_reply_th"] = "";
        }
        ...
        if( !empty($_GPC["comment_reply_th"]) )
        {
            $comment_reply_th = array(  );
            foreach( $_GPC["comment_reply_th"] as $reply_th )
            {
                $reply_th = trim($reply_th);
                if( empty($reply_th) )
                {
                ...
                $comment_reply_th[] = $reply_th;
            }
            $data["comment_reply_th"] = iserializer($comment_reply_th);
        }
        else
        {
            $data["comment_reply_th"] = iserializer(array(  ));
        }
        ...
        $data["order_note_th"] = array(  );
        if( !empty($_GPC["order_note_th"]) )
        {
            foreach( $_GPC["order_note_th"] as $order_note_th )
            {
                if( empty($order_note_th) )
                {
                    continue;
                }

                $data["order_note_th"][] = $order_note_th;
            }
        }

        $data["order_note_th"] = iserializer($data["order_note_th"]);
        ...
        if( !empty($_GPC["custom_title"]) )
        {
            ...
            {
                $title = trim($title);
                $title_th = trim($_GPC["custom_title_th"][$key]);
                ...
                $custom_url[] = array( "title" => $title,'title_th'=> $title_th ,  "url" => $url, "wxapp_link" => $wxapp_link );
            }
        ...
        $service_titles_th = $_GPC["service_titles_th"];
        foreach( $service_titles_th as $key => &$val )
        {
            $val = trim($val);
            if( empty($val) )
            {
                unset($service_titles_th[$key]);
            }

        }
        store_set_data($id, "service_titles_th", $service_titles_th);
