	...
	if ($_W['ispost']) {
		...
		if (!(empty($_W['ismanager']))) {
			...
			if ($discount < $agent_charge) {
			 ...
			 else if ($discount < $plateform_charge) {
				$plateform_charge = $discount;
				$agent_charge = 0;
				$store_charge = 0;
			}
			 else if ($discount < ($plateform_charge + $agent_charge)) {
		...
		$activity = array(
			...
			'data'      => array(
				...
				'amount'         => $amount,
				'discount_bear'  => array('plateform_charge' => $plateform_charge, 'agent_charge' => $agent_charge, 'store_charge' => $store_charge)
				)
			);
