...
if( $ta == "post" ) 
{
    ...
    if( $_W["ispost"] ) 
    {
        ...
        if( !empty($_GPC["goods_id"]) ) 
        {
            ...
            {
                $temp = pdo_fetch("select a.id, a.price, b.bargain_id from " . tablename("tiny_wmall_goods") . " as a left join " . tablename("tiny_wmall_activity_bargain_goods") . " as b on a.id = b.goods_id where a.uniacid = :uniacid and a.id = :id", array( ":uniacid" => $_W["uniacid"], ":id" => $goods_id ));
        ...
        $activity = array( "uniacid" => $_W["uniacid"], "sid" => $sid, "title" => $title, "starttime" => strtotime($_GPC["time"]["start"]), "endtime" => strtotime($_GPC["time"]["end"]) + 86399, "type" => "bargain", "status" => 1 );
        ...
        {
            $data = array( "uniacid" => $_W["uniacid"], "agentid" => $_W["agentid"], "bargain_id" => $id, "sid" => $store["id"], "goods_id" => $row["goods_id"], "discount_price" => $row["discount_price"], "max_buy_limit" => $row["max_buy_limit"], "poi_user_type" => $row["poi_user_type"], "discount_total" => $row["discount_total"], "discount_available_total" => $row["discount_available_total"] );
    ...
    if( empty($bargain) ) 
    {
        $bargain = array( "starttime" => TIMESTAMP, "endtime" => TIMESTAMP + 86400 * 15, "starthour" => "08:00", "endhour" => "12:00", "goods_limit" => 1, "order_limit" => 1, "goods" => array(  ) );
    }
...
if( $ta == "list" ) 
{
    ...
    $psize = 20;
    $total = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename("tiny_wmall_activity_bargain") . " where uniacid = :uniacid and agentid = :agentid and  sid = :sid and type = :type", array( ":uniacid" => $_W["uniacid"], ":agentid" => $_W["agentid"], ":sid" => $store["id"], ":type" => "bargain" ));
    $bargains = pdo_fetchall("select * from" . tablename("tiny_wmall_activity_bargain") . " where uniacid = :uniacid and agentid = :agentid and sid = :sid and type = :type order by id desc limit " . ($pindex - 1) * $psize . "," . $psize, array( ":uniacid" => $_W["uniacid"], ":agentid" => $_W["agentid"], ":sid" => $store["id"], ":type" => "bargain" ), "id");
...
if( $ta == "del" ) 
{
    ...
    $bargain = pdo_get("tiny_wmall_activity_bargain", array( "uniacid" => $_W["uniacid"], "agentid" => $_W["agentid"], "status" => 1, "sid" => $sid ));
    if( empty($bargain) ) 
    {
