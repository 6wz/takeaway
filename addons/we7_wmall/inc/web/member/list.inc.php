...
if ($op == 'setmeal') {
	...
	if ($_W['ispost']) {
		...
		mlog(6005, intval($_GPC['uid']), '套餐id：' . $_GPC['setmeal'] . '， 套餐开始-结束：' . $_GPC['setmeal_starttime'] . '-' . $_GPC['setmeal_endtime'] . '，每天可享受免费配送次数：' . $_GPC['free'] . '， 每单最多：' . $_GPC['deliveryfee'] . '元');
