...
if ($op == 'list') {
	...
	if (checksubmit()) {
		if (!(empty($_GPC['ids']))) {
			foreach ($_GPC['ids'] as $k => $v ) {
				...
				pdo_update("tiny_wmall_plugincenter_plugin", $data, array('uniacid' => $_W['uniacid'], 'id' => intval($v)));
			}
		}
	...
	if ($_W['ispost']) {
		$data = array('uniacid' => $_W['uniacid'], 'displayorder' => intval($_GPC['displayorder']), 'plugintype' => intval($_GPC['plugintype']), 'title' => trim($_GPC['title']), 'thumb' => trim($_GPC['thumb']), 'sailed' => intval($_GPC['sailed']), 'description' => htmlspecialchars_decode($_GPC['description']), 'status' => intval($_GPC['status']), 'addtime' => TIMESTAMP);
	...
	pdo_update("tiny_wmall_plugincenter_plugin", array("status" => $status), array('uniacid' => $_W['uniacid'], 'id' => $id));
	imessage(error(0, ''), '', "ajax");
}
 else if ($op == 'del') {
	$id = intval($_GPC['id']);
	pdo_delete("tiny_wmall_plugincenter_plugin", array("uniacid" => $_W['uniacid'], 'id' => $id));
