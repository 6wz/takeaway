...
if ($op == 'list') {
	...
	if (checksubmit()) {
		if (!(empty($_GPC['ids']))) {
			foreach ($_GPC['ids'] as $k => $v ) {
				...
				pdo_update("tiny_wmall_plugincenter_package", $data, array('uniacid' => $_W['uniacid'], 'id' => intval($v)));
			}
		}
	...
	if (0 < $id) {
		$pluginpackage = pdo_get('tiny_wmall_plugincenter_package', array('uniacid' => $_W['uniacid'], 'id' => $id));
	...
	if ($_W['ispost']) {
		$data = array('uniacid' => $_W['uniacid'], 'displayorder' => intval($_GPC['displayorder']), 'title' => trim($_GPC['title']), 'thumb' => trim($_GPC['thumb']), 'sailed' => intval($_GPC['sailed']), 'content' => trim($_GPC['content']), 'description' => htmlspecialchars_decode($_GPC['description']), 'status' => intval($_GPC['status']), 'addtime' => TIMESTAMP);
		...
		if (0 < $id) {
			...
			pdo_update("tiny_wmall_plugincenter_package", $data, array('uniacid' => $_W['uniacid'], 'id' => $id));
		}
		 else {
	...
	pdo_update("tiny_wmall_plugincenter_package", array("status" => $status), array('uniacid' => $_W['uniacid'], 'id' => $id));
	imessage(error(0, ''), '', "ajax");
}
 else if ($op == 'del') {
	$id = intval($_GPC['id']);
	pdo_delete("tiny_wmall_plugincenter_package", array("uniacid" => $_W['uniacid'], 'id' => $id));
