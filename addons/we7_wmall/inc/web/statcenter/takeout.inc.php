...
if( $op == "index" ) 
{
    ...
    if( $_W["isajax"] ) 
    {
        ...
        $chart = array( "stat" => $stat, "fields" => array( "total_success_order", "total_fee", "final_fee", "plateform_delivery_fee", "plateform_serve_fee", "plateform_deliveryer_fee", "plateform_discount_fee", "agent_discount_fee", "refund_fee", "extra_fee" ), "titles" => array( "有效订单量", "营业总额", "总入账", "平台配送费收入", "佣金收入", "配送员配送费支出", "平台补贴", "代理补贴", "总退款", "附加费" ) );
        if( $perm_zhunshibao ) 
        {
