...
{
    $data = array( "delivery_type" => 1, "credit2" => $deliveryer["credit2"], "rule" => $config_cash );
...
if( $op == "getcash_submit" ) 
{
    ...
    if( $_W["we7_wmall"]["config"]["getcash"]["channel"]["wechat"] == "wxapp" ) 
    {
        ...
        {
            message(ierror(-1, "未获取到配送员针对小程序的openid, 你可以尝试进入平台小程序会员中心来解决此问题"), "", "ajax");
        }
    ...
    $data = array( "uniacid" => $_W["uniacid"], "agentid" => $_W["agentid"], "deliveryer_id" => $deliveryer["id"], "trade_no" => date("YmdHis") . random(10, true), "get_fee" => $get_fee, "take_fee" => $take_fee, "final_fee" => $final_fee, "account" => iserializer(array( "nickname" => $deliveryer["nickname"], "openid" => $openid, "openid_wxapp" => $deliveryer["openid_wxapp"], "avatar" => $deliveryer["avatar"], "realname" => $deliveryer["title"] )), "status" => 2, "channel" => $channel, "addtime" => TIMESTAMP );
    ...
    {
        message(ierror(0, "申请提现成功,等待平台管理员审核", array( "getcash_id" => $getcash_id )), "", "ajax");
    }
    else
    {
        if( $getcashperiod == 1 ) 
        {
            ...
            message(ierror(0, $transfers["message"], array( "getcash_id" => $getcash_id )), "", "ajax");
        }
