...
if( $ta == "list" )
{
    ...
    if( !empty($stores) )
    {
        ...
        foreach( $stores as $key => &$row )
        {
            ...
            {
                $row["activity"]["items"]["delivery"] = array( "title" => language('满{delivery_free_price}免配送费' , ['delivery_free_price'=>$row["delivery_free_price"]]), "type" => "delivery" );
                $row["activity"]["num"] += 1;
            }
