...
if( $ta == "submit" ) 
{
    ...
    $activityed = order_count_activity($sid, $cart, $recordid, 0, 0, 0, 4);
    $order = array( "uniacid" => $_W["uniacid"], "acid" => $_W["acid"], "sid" => $sid, "uid" => $_W["member"]["uid"], "ordersn" => date("YmdHis") . random(6, true), "serial_sn" => store_order_serial_sn($sid), "code" => random(4, true), "order_type" => 4, "openid" => $_W["openid"], "mobile" => trim($_GPC["mobile"]), "username" => trim($_GPC["username"]), "person_num" => 0, "table_cid" => $cid, "reserve_type" => $reserve_type, "reserve_time" => (string) $date . " " . $time, "sex" => "", "address" => "", "location_x" => "", "location_y" => "", "delivery_day" => "", "delivery_time" => "", "delivery_fee" => 0, "pack_fee" => 0, "pay_type" => trim($_GPC["pay_type"]), "num" => $cart["num"], "price" => $cart["price"], "total_fee" => $cart["price"], "discount_fee" => $activityed["total"], "store_discount_fee" => $activityed["store_discount_fee"], "plateform_discount_fee" => $activityed["plateform_discount_fee"], "final_fee" => $cart["price"] - $activityed["total"], "status" => 1, "is_comment" => 0, "invoice" => trim($_GPC["invoice"]), "addtime" => TIMESTAMP, "data" => iserializer($cart["data"]), "note" => trim($_GPC["note"]) );
