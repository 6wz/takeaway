...
{
    imessage(language("该门店暂未开启买单功能"), "", "error");
}
...
if( $ta == "index" ) 
{
    if( $_W["isajax"] ) 
    {
        ...
        if( empty($total_fee) ) 
        {
            imessage(error(-1, language("消费总额不能为空")), "", "ajax");
        }
        ...
        if( 1 < $coupon["status"] ) 
        {
            imessage(error(-1, language("优惠券无效,请重新选择")), "", "ajax");
        }
        ...
        imessage(error(0, language("付款成功")), imurl("system/paycenter/pay", array( "sid" => $sid, "order_type" => "paybill", "id" => $order_id )), "ajax");
    }
