...
{
    imessage(language("门店未设置经纬度，需要完善经纬度后才能下单"), imurl("wmall/home/index"), "info");
}
...
if( $ta == "index" )
{
    ...
    if( !$delivery_time["reserve"] )
    {
        ...
        if( !empty($data) )
        {
            ...
            $text_time = $predict_time = language("尽快送达");
        ...
        {
            $predict_delivery_price = language('{predict_delivery_price}元配送费' ,['predict_delivery_price'=>$predict_delivery_price]);
        }
        else
        {
            $predict_delivery_price = language('配送费{predict_delivery_price}元起' , ['predict_delivery_price'=>$predict_delivery_price]);
        }
...
if( $ta == "submit" )
{
            ...
            else
            {
                ...
                else
                {
                    if( $store["delivery_fee_mode"] == 3 )
                    {
                        ...
                        if( $cart["price"] + $cart["box_price"] < $send_price )
                        {
                            imessage(error(-1, language("当前商品不满起送价")), "", "ajax");
                        }
    ...
    if( $order["delivery_time"] != language("尽快送出" ,'','zh-cn') &&  $order["delivery_time"] != language("尽快送出" ,'','th-th') )
    {
