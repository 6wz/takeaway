...
if( $ta == "list" )
{
    ...
    if( !empty($orders) )
    {
        ...
        foreach( $orders as &$da )
        {
            ...
            $da["comment_cn"] = language("去评价");
            if( $share["status"] == 1 && $da["status"] == 5 && $da["is_comment"] == 0 && time() - $share["share_grant_days_limit"] * 86400 <= $da["endtime"] )
            {
                $da["comment_cn"] = "<span class=\"color-danger\">". language('评价赢好礼') ."</span>";
            }
...
{
    $id = intval($_GPC["min"]);
    $orders = pdo_fetchall("select a.id as aid, a.*, b.title, b.logo, b.delivery_mode from " . tablename("tiny_wmall_order") . " as a left join " . tablename("tiny_wmall_store") . " as b on a.sid = b.id where a.uniacid = :uniacid and a.uid = :uid and a.is_delete = 0 and a.id < :id order by a.id desc limit 15", array( ":uniacid" => $_W["uniacid"], ":uid" => $_W["member"]["uid"], ":id" => $id ), "aid");
    $min = 0;
    if( !empty($orders) )
    {
        $share = get_plugin_config("ordergrant.share");
        ...
        foreach( $orders as &$order )
        {
            ...
            $order["comment_cn"] = language("去评价");
            if( $share["status"] == 1 && $order["status"] == 5 && $order["is_comment"] == 0 && time() - $share["share_grant_days_limit"] * 86400 <= $order["endtime"] )
            {
                $order["comment_cn"] ="<span class=\"color-danger\">". language('评价赢好礼') ."</span>";
            }
...
if( $ta == "detail" )
{
    ...
    if( check_plugin_perm("superRedpacket") )
    {
        ...
        if( !empty($superRedpacket) && 0 < $superRedpacket["packet_dosage"] )
        {
            ...
            if( !empty($superRedpacket_share) )
            {
                ...
                $_share = array( "title" => $superRedpacket_share["data"]["share"]["title"], "desc" => $superRedpacket_share["data"]["share"]["desc"], "imgUrl" => tomedia($superRedpacket_share["data"]["share"]["imgUrl"]), "link" => imurl("superRedpacket/share/index", array( "order_id" => $id ), true) );
            }
    ...
    $order_status = order_status();
    $slides = sys_fetch_slide("orderDetail");
...
if( $ta == "remind" )
{
    ...
    {
        imessage(error(-1, language("下单后{remind_time_start}分钟才可以催单" , ['remind_time_start'=>$remind_time_start])), "", "ajax");
    }
    ...
    {
        imessage(error(-1, language("距离上次催单不超过{remind_time_limit}分钟,不能催单" , ['remind_time_limit'=>$remind_time_limit])), "", "ajax");
    }
...
if( $ta == "cancel" )
{
    ...
    if( $order["status"] != 1 )
    {
        imessage(error(-1, language("商户已接单,如需取消订单请联系商户处理")), "", "ajax");
    }
    ...
    {
        imessage(error(-1, language("该订单已支付,如需取消订单请联系商户处理")), "", "ajax");
    }

    imessage(error(0, language("取消订单成功")), referer(), "ajax");
}
...
if( $ta == "end" )
{
    ...
    imessage(error(0, language("确认订单完成成功")), imurl("wmall/order/comment", array( "id" => $id )), "ajax");
}
...
if( $ta == "deleteOrder" )
{
    ...
    if( !$id )
    {
        imessage(error(-1, language("删除订单失败")), "", "ajax");
    }

    pdo_update("tiny_wmall_order", array( "is_delete" => 1 ), array( "uniacid" => $_W["uniacid"], "id" => $id ));
    imessage(error(0, language("删除订单成功")), imurl("wmall/order/index"), "ajax");
}
?>
