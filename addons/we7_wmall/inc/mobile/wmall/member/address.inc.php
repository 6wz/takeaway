...
if( $ta == "post" )
{
    ...
    if( 0 < $id )
    {
        ...
        if( empty($address) )
        {
            imessage(language("地址不存在或已经删除"), referer(), "error");
        }
    ...
    if( $_W["ispost"] )
    {
        ...
        {
            imessage(error(-1, language("信息有误")), "", "ajax");
        }
        ...
        {
            if( $store["order_address_limit"] == 2 )
            {
                ...
                {
                    imessage(error(-1, language('商户配送范围{serve_radius}公里, 当前地址不在商户配送范围内' ,['serve_radius'=>$store["serve_radius"]])), "", "ajax");
                }
            ...
            else
            {
                if( $store["order_address_limit"] == 4 )
                {
                    ...
                    {
                        imessage(error(-1, language("商户没有完善配送区域")), "", "ajax");
                    }
                    ...
                    if( empty($flag) )
                    {
                        imessage(error(-1, language("当前地址不在商户的配送范围")), "", "ajax");
                    }
