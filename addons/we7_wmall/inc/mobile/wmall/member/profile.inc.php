...
if( $ta == "edit" )
{
    ...
    if( $_W["ispost"] )
    {
        ...
        if( $type == "username" )
        {
            if( !$_GPC["realname"] )
            {
                imessage(error(-1, language("用户名不能空")), "", "ajax");
            }
        ...
        else
        {
            if( $type == "account" )
            {
                ...
                if( empty($password) )
                {
                    imessage(error(-1, language("密码不能为空")), "", "ajax");
                }

                if( empty($newpassword) )
                {
                    imessage(error(-1, language("新密码不能为空")), "", "ajax");
                }

                $length = strlen($newpassword);
                if( $length < 8 || 20 < $length )
                {
                    imessage(error(-1, language('请输入8-20位密码')), "", "ajax");
                }
                ...
                if( empty($repassword) )
                {
                    imessage(error(-1, language("请确认密码")), "", "ajax");
                }

                if( $newpassword != $repassword )
                {
                    imessage(error(-1, language("两次输入的密码不一致")), "", "ajax");
                }
                ...
                {
                    imessage(error(-1, language("原密码错误")), "", "ajax");
                }
        ...
        imessage(error(0, language("修改成功")), "", "ajax");
    }
...
if( $ta == "bind" )
{
    ...
    if( $_W["isajax"] )
    {
        $mobile = (trim($_GPC["mobile"]) ? trim($_GPC["mobile"]) : imessage(error(-1, language("请输入手机号")), "", "ajax"));
        $password = (trim($_GPC["password"]) ? trim($_GPC["password"]) : imessage(error(-1, language("请输入密码")), "", "ajax"));
        $length = strlen($password);
        if( $length < 8 || 20 < $length )
        {
            imessage(error(-1, language(language('请输入8-20位密码'))), "", "ajax");
        }
        ...
        $repassword = (trim($_GPC["repassword"]) ? trim($_GPC["repassword"]) : imessage(error(-1, language("请重复输入密码")), "", "ajax"));
        ...
        if( !empty($member) )
        {
            imessage(error(-1, language("该手机号已被其他用户绑定")), "", "ajax");
        }
        ...
        imessage(error(0, language("绑定成功")), "", "ajax");
    }
