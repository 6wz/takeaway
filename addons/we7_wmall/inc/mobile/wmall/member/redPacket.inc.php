...
if( $ta == "list" )
{
    ...
    if( !empty($redPackets) )
    {
        foreach( $redPackets as &$row )
        {
            ...
            {
                $row["time_cn"] = language('仅限{time_cn}时段使用' , ['time_cn'=>$row["time_cn"]]);
            }

            $row["category_cn"] = tocategory($row["category_limit"]);
            if( !empty($row["category_cn"]) )
            {
                $row["category_cn"] = language('仅限{category_cn}分类使用' , ['category_cn'=>$row["category_cn"]]);
            }
