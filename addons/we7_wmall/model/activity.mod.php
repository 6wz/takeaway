...
{
    global $_W;
    $type_basic = array( "discount", "newMember", "grant", "mallNewMember", "selfDelivery", "deliveryFeeDiscount", "cashGrant" );
    ...
    $params = array( ":uniacid" => $_W["uniacid"], ":time" => TIMESTAMP, ":hour" => date("Hi") );
    ...
    $params = array( ":uniacid" => $_W["uniacid"], ":time" => TIMESTAMP, ":status" => 1 );
...
function activity_store_cron($sid)
{
    ...
    $params = array( ":uniacid" => $_W["uniacid"], ":sid" => $sid, ":time" => TIMESTAMP, ":hour" => date("Hi") );
    $params[":hour"] = ltrim($params[":hour"], "0");
    ...
    $params = array( ":uniacid" => $_W["uniacid"], ":time" => TIMESTAMP, ":status" => 1, ":sid" => $sid );
...
function activity_del($sid, $type = "")
{
    ...
    if( empty($type) ) 
    {
        if( $_W["role"] == "clerker" ) 
        {
            return error(-1, "您没有删除活动的权限，请联系平台管理员删除");
        }
    ...
    {
        if( $_W["role"] == "clerker" && $_W["we7_wmall"]["config"]["store"]["activity"]["perm"][$type]["cancel_status"] != 1 ) 
        {
            return error(-1, "您没有删除该活动的权限，请联系平台管理员删除");
        }
    ...
    {
        if( $type == "svipRedpacket" ) 
        {
            pdo_delete("tiny_wmall_svip_redpacket", array( "uniacid" => $_W["uniacid"], "sid" => $sid ));
        }
...
{
    $data = array( "1" => array( "css" => "label label-success", "text" => "进行中", "color" => "color-success" ), "0" => array( "css" => "label label-danger", "text" => "活动未开始或已结束", "color" => "color-danger" ) );
    return $data;
}
