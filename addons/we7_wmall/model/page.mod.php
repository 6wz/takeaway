...
function store_filter($filter = array(  ), $orderby = "")
{
    ...
    $stores = pdo_fetchall("select id,agentid,score,title,logo,content,sailed,score,label,delivery_type,serve_radius,not_in_serve_radius,delivery_areas,business_hours,is_in_business,is_rest,is_stick,delivery_fee_mode,delivery_price,delivery_free_price,send_price,delivery_time,delivery_mode,token_status,invoice_status,location_x,location_y,forward_mode,forward_url,displayorder,click,\r\n ROUND(\r\n        6378.138 * 2 * ASIN(\r\n            SQRT(\r\n                POW(\r\n                    SIN(\r\n                        (\r\n                            " . $lat . " * PI() / 180 - location_x * PI() / 180\r\n                        ) / 2\r\n                    ),\r\n                    2\r\n                ) + COS(" . $lat . " * PI() / 180) * COS(location_x * PI() / 180) * POW(\r\n                    SIN(\r\n                        (\r\n                           " . $lng . "  * PI() / 180 - location_y * PI() / 180\r\n                        ) / 2\r\n                    ),\r\n                    2\r\n                )\r\n            )\r\n        ) * 1000) as distance from " . tablename("tiny_wmall_store") . " " . $condition . " " . $order_by_base . " " . $limit, $params, "id");
...
function store_page_get($sid, $id = 0)
{
    ...
    if( !empty($page) ) 
    {
        ...
        {
            if( in_array($item["id"], array( "picture", "operation", "banner" )) ) 
            {
