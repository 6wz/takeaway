...
function print_add_order($printDeviceInfo = array(  ), $order = array(  ))
{
    ...
    $notify_url = ((WE7_WMALL_ISHTTPS && $notify_use_http ? WE7_WMALL_URL_NOHTTPS : WE7_WMALL_URL)) . "payment/print/notify.php";
    ...
    {
        $postdata = array( "sn" => $printDeviceInfo["deviceno"], "key" => $printDeviceInfo["key"], "printContent" => implode("<BR>", $printDeviceInfo["content"]), "times" => $printDeviceInfo["times"], "backurl" => $notify_url );
    ...
    else
    {
        if( $printDeviceInfo["type"] == "feie_new" ) 
        {
            ...
            $postdata = array( "user" => $user, "stime" => TIMESTAMP, "sig" => $sign, "apiname" => "Open_printMsg", "sn" => $printDeviceInfo["deviceno"], "content" => implode("<BR>", $printDeviceInfo["content"]), "times" => $printDeviceInfo["times"], "backurl" => $notify_url, "callbackurl" => $notify_url );
            $posturl = "http://api.feieyun.cn/Api/Open/";
        }
        else
        {
            if( $printDeviceInfo["type"] == "feiyin" ) 
            {
            ...
            {
                if( $printDeviceInfo["type"] == "AiPrint" ) 
                {
                ...
                else
                {
                    ...
                    {
                        if( $printDeviceInfo["type"] == "yilianyun" ) 
                        {
                            ...
                            $time = time();
                            $sign = strtoupper(md5((string) $printDeviceInfo["api_key"] . "machine_code" . $printDeviceInfo["deviceno"] . "partner" . $printDeviceInfo["member_code"] . "time" . $time . $printDeviceInfo["key"]));
                        ...
                        else
                        {
                            ...
                            else
                            {
                                if( $printDeviceInfo["type"] == "xixun" ) 
                                {
                                    ...
                                    foreach( $printDeviceInfo["content"] as &$v ) 
                                    {
                                        //$printDeviceInfo["key"] = (U:0         );
                                    ...
                                    $postdata = array( "dingdan" => $printDeviceInfo["content"], "dingdanID" => $order["print_sn"], "dayinjisn" => $printDeviceInfo["deviceno"], "pages" => $printDeviceInfo["times"], "replyURL" => $notify_url );
                                }
                                else
                                {
...
{
    $data = array( "feie" => array( "printorder" => array( "服务器接收订单成功", "打印机编号错误", "服务器处理订单失败", "打印内容太长", "请求参数错误" ), "qureyorder" => array( "已打印/未打印", "请求参数错误", "服务器处理订单失败", "没有找到该索引的订单" ), "qureystate" => array(  ) ), "feiyin" => array( "printorder" => array( "正常", "-1" => "IP地址不允许", "-2" => "关键参数为空或请求方式不对", "-3" => "客户编码不对", "-4" => "安全校验码不正确", "-5" => "请求时间失效", "-6" => "订单内容格式不对", "-7" => "重复的消息 （ msgNo 的值重复）", "-8" => "消息模式不对", "-9" => "服务器错误", "-10" => "服务器内部错误", "-111" => "打印终端不属于该账户" ), "qureyorder" => array( "打印请求/任务中队列中，等待打印", "打印任务已完成/请求数据已打印", "打印任务/请求失败", "9" => "打印任务/请求已发送", "-1" => "IP地址不允许", "-2" => "关键参数为空或请求方式不对", "-3" => "客户编码不正确", "-4" => "安全校验码不正确", "-5" => "请求时间失效。请求时间和请求到达飞印API的时间长超出安全范围。", "-6" => "订单编号错误或者不存在" ), "qureystate" => array( "-1" => "IP地址不允许", "-2" => "关键参数为空或请求方式不对", "-3" => "客户编码不正确", "-4" => "安全校验码不正确", "-5" => " 同步应用服务器时间 了解更多飞印API的时间安全设置。" ) ), "365" => array( "printorder" => array( "正常", "2" => "订单添加成功，但是打印机缺纸，无法打印", "3" => "订单添加成功，但是打印机不在线", "10" => "内部服务器错误", "11" => "参数不正确", "12" => "打印机未添加到服务器", "13" => "未添加为订单服务器", "14" => "订单服务器和打印机不在同一个组", "15" => "订单已经存在，不能再次打印" ), "qureyorder" => array( "打印成功", "正在打印中", "打印机缺纸", "打印机下线", "16" => "订单不存在" ), "qureystate" => array( "1" => "打印机正常在线", "2" => "打印机缺纸", "3" => "打印机下线", "4" => "错误的机器号或口令" ) ), "365_s2" => array( "printorder" => array( "正常", "2" => "订单添加成功，但是打印机缺纸，无法打印", "3" => "订单添加成功，但是打印机不在线", "10" => "内部服务器错误", "11" => "参数不正确", "12" => "打印机未添加到服务器", "13" => "未添加为订单服务器", "14" => "订单服务器和打印机不在同一个组", "15" => "订单已经存在，不能再次打印" ), "qureyorder" => array( "打印成功", "正在打印中", "打印机缺纸", "打印机下线", "16" => "订单不存在" ), "qureystate" => array( "1" => "打印机正常在线", "2" => "打印机缺纸", "3" => "打印机下线", "4" => "错误的机器号或口令" ) ), "yilianyun" => array( "printorder" => array( 1 => "数据提交成功", 2 => "提交时间超时。验证你所提交的时间戳超过3分钟后拒绝接受", 3 => "参数有误", 4 => "sign加密验证失败" ), "qureyorder" => array( "已打印/未打印", "请求参数错误", "服务器处理订单失败", "没有找到该索引的订单" ), "qureystate" => array(  ) ) );
    return $data;
}

function print_printer_types()
{
    return array( "feie" => array( "text" => "飞鹅打印机", "css" => "label label-success" ), "feie_new" => array( "text" => "飞鹅打印机新接口", "css" => "label label-success" ), "feiyin" => array( "text" => "飞印打印机", "css" => "label label-danger" ), "365" => array( "text" => "365 S1打印机", "css" => "label label-warning" ), "365_s2" => array( "text" => "365 S2打印机", "css" => "label label-success" ), "AiPrint" => array( "text" => "AiPrint打印机", "css" => "label label-default" ), "yilianyun" => array( "text" => "易联云打印机", "css" => "label label-primary" ), "qiyun" => array( "text" => "启云打印机", "css" => "label label-info" ), "xixun" => array( "text" => "喜讯打印机", "css" => "label label-info" ) );
}
?>