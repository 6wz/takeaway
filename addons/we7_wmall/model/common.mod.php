...
function check_perm($perm, $redirct = false)
{
    ...
    if( $redirct ) 
    {
        $perms_init = array( "dashboard.index", "merchant.store", "order.takeout", "statcenter.takeout", "paycenter.paybill", "merchant.store", "service.comment", "deliveryer.account", "clerk.account", "member.index", "config.mall", "errander.index", "bargain.index", "deliveryCard.index", "qianfanApp.index", "majiaApp.index", "shareRedpacket.index", "freeLunch.index", "diypage.index", "ordergrant.index", "superRedpacket.index", "creditshop.index", "agent.index", "wheel.index", "gohome.index" );
...
function get_all_perms($justkey = false)
{
    ...
    {
        $all_perms["spread"] = array( "title" => "LL推广", "perms" => array( "spread.members" => "推广员", "spread.user" => "推广关系", "spread.groups" => "推广员等级", "spread.getcash" => "提现记录", "spread.current" => "账户明细", "spread.config" => "基本设置", "spread.postera" => "推广海报", "spread.rank" => "排行榜设置", "spread.cover" => "入口设置" ) );
    }
    ...
    {
        $all_perms["gohome"] = array( "title" => "LL生活圈", "perms" => array( "gohome.slide" => "生活圈幻灯片", "gohome.nav" => "生活圈导航图标", "gohome.notice" => "生活圈公告", "gohome.order" => "订单列表", "kanjia.category" => "砍价活动分类", "kanjia.activity" => "砍价活动列表", "pintuan.category" => "拼团活动分类", "pintuan.activity" => "拼团活动列表", "pintuan.basic" => "拼团活动设置", "seckill.goods_category" => "抢购活动分类", "seckill.goods" => "抢购活动列表", "gohome.comment" => "订单评论", "gohome.complain" => "投诉列表", "gohome.memberBlack" => "黑名单", "gohome.config" => "活动设置/费率设置", "gohome.poster" => "活动海报", "gohome.cover" => "活动入口", "tongcheng.category" => "同城分类列表", "tongcheng.basic" => "同城设置", "tongcheng.slide" => "同城幻灯片", "tongcheng.information" => "同城帖子", "tongcheng.comment" => "同城评论列表", "haodian.settle" => "好店入驻设置", "haodian.slide" => "好店幻灯片", "haodian.category" => "好店商户分类", "haodian.store" => "好店商户列表" ) );
    }

    if( check_plugin_perm("svip") ) 
    {
        $all_perms["svip"] = array( "title" => "超级会员", "perms" => array( "svip.redpacket" => "超级会员专享红包", "svip.goods" => "超级会员专享商品", "svip.task" => "超级会员任务", "svip.setmeal" => "套餐列表", "svip.order" => "购买记录", "svip.config" => "会员设置", "svip.cover" => "入口设置" ) );
    }
