...
{
    global $_W;
    $elements = array( "Expiration", "PhoneNoA", "PoolKey" );
    $params = array_elements($elements, $params);
    $accessKeyId = "GgeKcqgxozCoG8I8";
    $accessSecret = "gBf4QQb5LL6lZsWbgQzr08oFEtY6kV";
    if( empty($params["Expiration"]) ) 
    {
        return error(-1, "请输入绑定关系的过期时间");
    }

    if( empty($params["PhoneNoA"]) ) 
    {
        return error(-1, "请输入要加密的电话号码");
    }

    if( empty($params["Expiration"]) ) 
    {
        return error(-1, "请输入号码池Key");
    }

    date_default_timezone_set("GMT");
    $public_params = array( "AccessKeyId" => $accessKeyId, "SignatureMethod" => "HMAC-SHA1", "SignatureVersion" => "1.0", "SignatureNonce" => uniqid(), "Timestamp" => date("Y-m-d\\TH:i:s\\Z"), "Action" => "BindAxnExtension", "Version" => "2017-05-25", "Format" => "json" );
...
{
    $msg = array( "isp.RAM_PERMISSION_DENY" => array( "msg" => "RAM权限DENY" ), "isv.OUT_OF_SERVICE" => array( "msg" => "业务停机" ), "isv.PRODUCT_UN_SUBSCRIPT" => array( "msg" => "未开通云通信产品的阿里云客户" ), "isv.ACCOUNT_NOT_EXISTS" => array( "msg" => "账户不存在" ), "isv.ACCOUNT_ABNORMAL" => array( "msg" => "账户异常" ), "isp.SYSTEM_ERROR" => array( "msg" => "isp.SYSTEM_ERROR" ), "isp.UNKNOWN_ERR_CODE" => array( "msg" => "运营商未知错误" ), "isv.PARTNER_NOT_EXIST" => array( "msg" => "未知合作伙伴" ), "isv.NO_NOT_EXIST" => array( "msg" => "号码不存在" ), "isv.ILLEGAL_ARGUMENT" => array( "msg" => "参数非法" ), "isp.DAO_EXCEPTION" => array( "msg" => "数据库异常" ), "isv.NO_AVAILABLE_NUMBER" => array( "msg" => "无可用号码" ), "isp.VENDOR_UNAVAILABLE" => array( "msg" => "运营商降级" ), "isv.FLOW_LIMIT" => array( "msg" => "业务流控" ), "isv.PARTNER_IS_CLOSED" => array( "msg" => "partner被关停" ), "isv.FORBIDDEN_ACTION" => array( "msg" => "无权操作" ), "isv.NO_USED_BY_OTHERS" => array( "msg" => "号码被其他业务方占用" ), "isv.VENDOR_BIND_FAILED" => array( "msg" => "运营商绑定失败" ), "isv.EXPIRE_DATE_ILLEGAL" => array( "msg" => "过期时间非法" ), "isv.MOBILE_NUMBER_ILLEGAL" => array( "msg" => "号码格式非法" ), "isv.BIND_CONFLICT" => array( "msg" => "绑定冲突" ) );
    return $msg[$code];
}
?>