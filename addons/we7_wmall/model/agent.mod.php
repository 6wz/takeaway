...
function agent_update_account($id, $fee, $trade_type, $extra, $remark = "", $order_type = "takeout")
{
    ...
    $hash = md5((string) $_W["uniacid"] . "-" . $id . "-" . $trade_type . "-" . $extra);
    ...
    if( !empty($id) ) 
    {
        pdo_update("tiny_wmall_agent", array( "amount" => $now_amount ), array( "uniacid" => $_W["uniacid"], "id" => $agent["id"] ));
    }
...
function get_agent_perms()
{
    $all_perms = array( "dashboard" => array( "title" => "概括", "perms" => array( "dashboard.index" => "运营概况", "dashboard.ad" => "全屏引导页", "dashboard.slide" => "幻灯片", "dashboard.nav" => "导航图标", "dashboard.notice" => "公告", "dashboard.cube" => "图片魔方" ) ), "order" => array( "title" => "订单", "perms" => array( "order.takeout" => "外卖", "order.takeoutNew" => "未完成", "order.distribute" => "订单分布", "order.neworder" => "代客下单", "order.dispatch" => "调度中心-待指派", "order.records" => "调度中心-接单统计/接单记录", "order.tangshi" => "店内" ) ), "paycenter" => array( "title" => "当面付", "perms" => array( "paycenter.paybill" => "买单订单" ) ), "statcenter" => array( "title" => "数据", "perms" => array( "statcenter.takeout" => "外卖统计", "statcenter.paytype" => "支付方式统计", "statcenter.takeoutOrder" => "店铺订单统计", "statcenter.takeoutOrderChannel" => "订单来源统计", "statcenter.delivery" => "配送统计/配送详情", "statcenter.hot" => "热门商品", "statcenter.finance" => "财务统计" ) ), "merchant" => array( "title" => "商户", "perms" => array( "merchant.store" => "商户列表", "merchant.account" => "商户账户", "merchant.activity" => "商户活动/活动展示", "merchant.getcash" => "申请提现", "merchant.current" => "账户明细", "merchant.settle" => "入驻", "merchant.storage" => "商家回收站", "merchant.newsCategory" => "资讯分类", "merchant.news" => "资讯列表", "merchant.ad" => "广告", "merchant.notice" => "公告列表", "merchant.report" => "投诉列表" ) ), "service" => array( "title" => "售后", "perms" => array( "service.comment" => "用户评价" ) ), "deliveryer" => array( "title" => "配送员", "perms" => array( "deliveryer.plateform" => "配送员", "deliveryer.getcash" => "提现申请", "deliveryer.current" => "账户明细", "deliveryer.comment" => "配送评价", "deliveryer.cover" => "注册&登录" ) ), "clerk" => array( "title" => "店员", "perms" => array( "clerk.account" => "店员列表", "clerk.cover" => "注册&登录" ) ), "agent.config" => array( "title" => "设置", "perms" => array( "agent.config.mall" => "基础设置/分享及关注/平台状态/oAuth设置", "agent.config.takeout" => "服务范围/订单相关", "agent.config.store" => "配送模式/服务费率/商户入驻/其他批量操作", "agent.config.deliveryer" => "配送员申请/提成及提现" ) ), "agent.finance" => array( "title" => "财务", "perms" => array( "agent.finance.getcash" => "提现" ) ), "agent" => array( "title" => "代理", "perms" => array( "agent.agent" => "代理", "agent.getcash" => "申请提现", "agent.current" => "账户明细" ) ), "errander" => array( "title" => "跑腿", "perms" => array( "errander.config" => "设置", "errander.order" => "订单" ) ) );
    $perms = array(  );
