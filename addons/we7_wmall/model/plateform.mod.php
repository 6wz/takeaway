...
function plateform_login($params)
{
	...
	$record['token'] = $type . ':' . $record['token'];
...
function plateform_push_token($user)
{
	...
	$relation = array(
		'alias' => $user['token'],
		'tags'  => array($config['push_tags']['all'])
		);

	if ($user['usertype'] == 'agenter') {
		if (!(empty($user['token']))) {
...
function icheckplateformer()
{
	...
	if (!(empty($token))) {
		...
		$perm = $_W['_action'] . '.' . $_W['_op'];

		if (!(empty($_W['_op_perm']))) {
			$perm = $_W['_action'] . '.' . $_W['_op_perm'];
		}
...
function plateform_urls()
{
	...
	$data['takeout']['sys'] = array(
		'title' => '外卖',
		'items' => array(
			array('title' => '外卖订单', 'url' => 'pages/order/takeout'),
			array('title' => '当面付', 'url' => 'pages/paycenter/paybill'),
			array('title' => '售后', 'url' => 'pages/service/comment?'),
			array('title' => '统计', 'url' => 'pages/statcenter/index')
			)
		);
	$data['store']['sys'] = array(
		'title' => '商户',
		'items' => array(
			array('title' => '商户列表', 'url' => 'pages/merchant/store'),
			array('title' => '商户活动列表', 'url' => 'pages/merchant/activity/list'),
			array('title' => '提现申请记录', 'url' => 'pages/merchant/getcash'),
			array('title' => '账户明细记录', 'url' => 'pages/merchant/current'),
			array('title' => '商户入驻列表', 'url' => 'pages/merchant/settle'),
			array('title' => '商家回收站', 'url' => 'pages/merchant/storage'),
			array('title' => '投诉列表', 'url' => 'pages/merchant/report')
			)
		);
	$data['deliveryer']['sys'] = array(
		'title' => '配送员',
		'items' => array(
			array('title' => '配送员管理', 'url' => 'pages/deliveryer/index'),
			array('title' => '配送员列表', 'url' => 'pages/deliveryer/deliveryer'),
			array('title' => '提现申请记录', 'url' => 'pages/deliveryer/getcash'),
			array('title' => '账户明细记录', 'url' => 'pages/deliveryer/current'),
			array('title' => '配送员位置', 'url' => 'pages/deliveryer/location')
			)
		);
	...
	if (check_plugin_perm("errander")) {
		$data['plugin']['errander'] = array(
			'title' => '跑腿',
			'items' => array(
				array('title' => '跑腿管理', 'url' => 'pages/plugin/paotui/index'),
				array('title' => '跑腿订单', 'url' => 'pages/plugin/paotui/list'),
				array('title' => '跑腿设置', 'url' => 'pages/plugin/paotui/config')
				)
			);
	}
	...
	if (check_plugin_perm("agent")) {
		$data['plugin']['agent'] = array(
			'title' => '区域代理',
			'items' => array(
				array('title' => '区域代理管理', 'url' => 'pages/plugin/agent/index'),
				array('title' => '代理列表', 'url' => 'pages/plugin/agent/agent'),
				array('title' => '提现记录', 'url' => 'pages/plugin/agent/getcash'),
				array('title' => '账户明细', 'url' => 'pages/plugin/agent/current')
				)
			);
	}
	...
	if (check_plugin_perm("creditshop")) {
		$data['plugin']['creditshop'] = array(
			'title' => '积分商城',
			'items' => array(
				array('title' => '兑换列表', 'url' => 'pages/plugin/creditshop/order')
				)
			);
	}
	...
	if (check_plugin_perm("deliveryCard")) {
		$data['plugin']['deliveryCard'] = array(
			'title' => '配送会员卡',
			'items' => array(
				array('title' => '购买记录', 'url' => 'pages/plugin/deliveryCard/order')
				)
			);
	}
	...
	if (check_plugin_perm("mealRedpacket")) {
		$data['plugin']['mealRedpacket'] = array(
			'title' => '套餐红包',
			'items' => array(
				array('title' => '购买记录', 'url' => 'pages/plugin/mealRedpacket/order')
				)
			);
	}
	...
	if (check_plugin_perm("wheel")) {
		$data['plugin']['wheel'] = array(
			'title' => '幸运大转盘',
			'items' => array(
				array('title' => '参与记录', 'url' => 'pages/plugin/wheel/record')
				)
			);
	}
	...
	if (check_plugin_perm("advertise")) {
		$data['plugin']['advertise'] = array(
			'title' => '商户广告通',
			'items' => array(
				array('title' => '购买记录', 'url' => 'pages/plugin/advertise/order')
				)
			);
	}
	...
	$data['other']['sys'] = array(
		'title' => '其他',
		'items' => array(
			array('title' => '顾客列表', 'url' => 'pages/member/list'),
			array('title' => '系统设置', 'url' => 'pages/config/index'),
			array('title' => '更多', 'url' => 'pages/more/index'),
			array('title' => '我的', 'url' => 'pages/member/mine')
			)
		);
	return $data;
}
...
function get_plateform_menu()
{
	...
	if (empty($menu)) {
		$menu = array(
			...
			'css'    => array('iconColor' => '#163636', 'iconColorActive' => '#4FAE52', 'textColor' => '#929292', 'textColorActive' => '#4FAE52'),
			'data'   => array(
				'M0123456789101' => array('link' => '/pages/order/takeout', 'icon' => 'icon-order', 'text' => '外卖'),
				'M0123456789102' => array('link' => '/pages/merchant/store', 'icon' => 'icon-shop', 'text' => '店铺'),
				'M0123456789103' => array('link' => '/pages/more/index', 'icon' => 'icon-mark1', 'text' => '更多'),
				'M0123456789104' => array('link' => '/pages/member/mine', 'icon' => 'icon-mine', 'text' => '我的')
				)
			);
	}
	 else {
