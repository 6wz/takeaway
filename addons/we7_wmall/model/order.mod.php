...
function order_clerk_notice($id, $type, $note = '', $extra = array()) {
	...
	if($type == 'place_order') {
		...
		$title = "{$order['table']['title']}桌客人加菜了, 订单号为{$order['ordersn']}, 请尽快处理";
...
function order_status_update($id, $type, $extra = array()) {
	...
	if($type == 'handle') {
		...
		if($store['auto_notice_deliveryer'] == 1 && empty($order['is_reserve'])) {
...
function order_update_bill($order_id, $extra_data = array()) {
	...
	$agent_serve['final'] = "(代理商抽取佣金 ￥{$platform_serve_fee} - 平台服务佣金 ￥{$agent_serve_fee} - 代理商补贴 ￥{$order['agent_discount_fee']} + 代理商配送费 ￥{$plateform_delivery_fee} - 代理商支付给配送员配送费 ￥{$plateform_deliveryer_fee}{$zhunshibao_cn})";
	$data = array(
