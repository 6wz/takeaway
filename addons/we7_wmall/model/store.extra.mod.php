...
function store_getcash_update($logOrId, $type, $extra = array())
{
	...
	if ($type == 'transfers') {
		...
		if (($log['channel'] == 'weixin') || ($log['channel'] == 'wxapp')) {
		 ...
		 else if ($log['channel'] == 'alipay') {
			mload()->classs('alipay');
			$pay = new AliPay();
			$params = array('out_biz_no' => $log['trade_no'], 'payee_account' => $log['account']['account'], 'amount' => $log['final_fee'], 'payee_real_name' => $log['account']['realname'], 'remark' => $store['title'] . \date('Y-m-d H:i', $log['addtime']) . '提现申请');
			$response = $pay->transfer($params);
		}
		 else if ($log['channel'] == 'bank') {
			mload()->classs('wxpay');
			$pay = new WxPay();
			$params = array('partner_trade_no' => $log['trade_no'], 'enc_bank_no' => $log['account']['account'], 'enc_true_name' => $log['account']['realname'], 'bank_code' => $log['account']['id'], 'amount' => $log['final_fee'] * 100, 'desc' => $store['title'] . \date('Y-m-d H:i', $log['addtime']) . '提现申请');
