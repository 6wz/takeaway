...
function goods_avaliable_fetchall($sid, $cid = 0, $ignore_bargain = false)
{
    ...
    $params = array( ":uniacid" => $_W["uniacid"], ":sid" => $sid );
    $goods = pdo_fetchall("select id, cid, title, price, ts_price, box_price,svip_status, svip_price, total, thumb, sailed, label, content, is_options, attrs, unitname, unitnum, comment_good, status, is_showtime, start_time1, end_time1, start_time2, end_time2, week from " . tablename("tiny_wmall_goods") . $condition, $params, "id");
    if( empty($goods) ) 
    {
        return $result;
    }
