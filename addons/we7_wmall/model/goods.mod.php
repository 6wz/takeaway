...
function goods_format($goods)
{
    ...
    $goods["unitname_cn_th"] = (!empty($goods["unitname_th"]) ? "/" . $goods["unitname_th"] : "");
    $goods["options"] = array(  );
...
function goods_avaliable_fetchall($sid, $cid = 0, $ignore_bargain = false)
{
    ...
    $params = array( ":uniacid" => $_W["uniacid"], ":sid" => $sid );
    $goods = pdo_fetchall("select id, cid, title,title_th , price, ts_price, box_price,svip_status, svip_price, total, thumb, sailed, label,label_th, content,content_th , is_options, attrs, attrs_th ,unitname,unitname_th, unitnum, comment_good, status, is_showtime, start_time1, end_time1, start_time2, end_time2, week from " . tablename("tiny_wmall_goods") . $condition, $params, "id");
    if( empty($goods) )
    {
        return $result;
    }
    ...
    foreach( $goods as &$good )
    {
        ...
        $good["unitname_cn_th"] = (!empty($good["unitname_th"]) ? "/" . $good["unitname_th"] : "");
        $good["options"] = array(  );
    ...
    if( defined("IN_VUE") || defined("IN_WXAPP") )
    {
        ...
        if( !empty($categorys) )
        {
            foreach( $categorys as $key => $value )
            {
                ...
                {
                    $cate_has_goods[] = array( "id" => $value["id"], "title" => $value["title"], "category_min_fee" => $value["min_fee"], "total" => $value["total"], "goods" => $cate_goods[$value["id"]] );
                }
...
function goods_build_options($goods)
{
    ...
    {
        $goods["options"] = array( array( "id" => "0", "name" => "", "price" => $goods["price"], "total" => $goods["total"], "svip_status" => $goods["svip_status"], "svip_price" => $goods["svip_price"] ) );
...
function goods_filter($sid, $filter = array(  ))
{
    ...
    if( strexists($filter["cid"], "bargain_") )
    {
        ...
        $where = " as a left join " . tablename("tiny_wmall_goods") . " as b on a.goods_id = b.id where a.uniacid = :uniacid and a.sid = :sid and (a.discount_available_total = -1 or a.discount_available_total > 0) and a.bargain_id = :bargain_id";
        if( ORDER_TYPE == "takeout" )
        {
