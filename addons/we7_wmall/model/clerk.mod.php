...
function clerk_fetchall($sid)
{
    ...
    if( !empty($data) ) 
    {
        foreach( $data as &$row ) 
        {
            ...
            if( empty($row["extra"]) ) 
            {
                $row["extra"] = array( "accept_wechat_notice" => 1, "accept_voice_notice" => 1 );
            }
...
function clerk_info_init()
{
    ...
    if( !empty($clerks) ) 
    {
        $update = array( "updatetime" => TIMESTAMP );
...
function clerk_push_token($clerk_id)
{
    ...
    $relation = array( "alias" => $clerk["token"], "tags" => array(  ) );
...
function clerk_set_extra($type, $value, $clerk_id = 0, $sid = 0)
{
    ...
    if( !empty($data) ) 
    {
        if( empty($data["extra"]) ) 
        {
            ...
            {
                $extra = array( "accept_wechat_notice" => $value, "accept_voice_notice" => 0 );
            }
            else
            {
                if( $type == "accept_voice_notice" ) 
                {
                    $extra = array( "accept_wechat_notice" => 0, "accept_voice_notice" => $value );
                }
