		...
		<div class="panel-body table-responsive">
			...
			<?php  if(is_array($records)) { foreach($records as $record) { ?>
				<tr>
					...
					<?php  } ?>
					<td><span class="text-danger">￥<?php  echo $record['extra_fee'];?></span></td>
...
irequire(['echarts'], function(echarts){
	var is_agent = <?php  echo $_W['is_agent'];?>;
	var perm_zhunshibao = <?php  echo $perm_zhunshibao;?>;
	var option = {
		title: {
			text: '营业趋势图'
		},
	...
	$.post(location.href, function(data){
		...
		$.each(result.message.message.fields, function(k, v){
			if((v == 'agent_discount_fee' || v == 'agent_serve_fee') && !is_agent) {
				return true;
			}
			var serie = {
		...
		if(is_agent) {
			...
			$('#html-total-agent-discount-fee').html('￥' + stat.agent_discount_fee);
		}
		if(perm_zhunshibao) {
